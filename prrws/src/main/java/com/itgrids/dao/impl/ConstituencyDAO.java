...
public class ConstituencyDAO extends GenericDaoHibernate<Constituency, Long> implements IConstituencyDAO {
	...
	public List<Object[]> getConstituencyIdFromPRConstituencyId(String locationIdStr){
		Query query = getSession().createQuery("select distinct model.constituencyId,model.districtId from Constituency model where model.prConstituency.constituencyCode = :locationIdStr");
		query.setParameter("locationIdStr", locationIdStr);
		return query.list();
	}
	
	public Object[] getConstituencyDetails(Long constituencyId){
		//0-constId,1-constName,2-distId,3-distName,4-stateId,5-stateName
		Query query = getSession().createQuery(" select model.constituencyId,model.name,model.districtId,model.district.districtName,model.stateId,model.state.stateName "
				+ " from Constituency model "
	...
	public List<Object[]> getConstituencyDetails1(Long constituencyId){
		Query query = getSession().createQuery("select model.districtId,model.district.districtName,model.constituencyId,model.name" +
				" from Constituency model" +
	...
	@Override
	public List<Object[]> getPrEncConstituencies(Long locationId) {
		Query query = getSession().createQuery("select model.encConstituencyId,model.name" +
				" from Constituency model" +
				" where model.district.encDistrictId=:locationId");
