...
function fetchDataForWordCloud(type) {
	...
	if(type =="fromPage"){
		...
		var requestObj = { "Constituency": constituencies,"News_Paper": globalNewsPapres, "Distrtict": districtNames, "Edition_Type":globalEditionType,"start_date": glStartDate, "end_date": glEndDate,
		"is_department":isDepartment,"department_name":departmentNames};
...
function getAllLocations(levelId,levelValue,type){
	if(levelId==2){
		...
		$('#wordCloudConstituency').html("ALL constituencies");
		var jsObj={
				"districtNames":levelValue
			}
		$.ajax({
			type:'POST',    
			url: 'getAllConstituenciesByDistrictName',
			dataType: 'json',
			data : JSON.stringify(jsObj),
			beforeSend :   function(xhr){
				xhr.setRequestHeader("Accept", "application/json");
