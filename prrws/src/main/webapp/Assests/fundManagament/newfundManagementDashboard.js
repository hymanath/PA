...
var $windowWidth = $(window).width();
	...
	function onLoadInitialisations(){
		...
		getSchemeWiseLocationWiseAmountDetails(2,'stateLevlOvervw','scheme','count','desc',0,0);
		getSchemeWiseLocationWiseAmountDetails(3,'distLevlOvervw','scheme','count','desc',0,3);
		getSchemeWiseLocationWiseAmountDetails(4,'consLevlOvervw','scheme','count','desc',0,4);
		getSchemeWiseLocationWiseAmountDetails(5,'mandalLevlOvervw','scheme','count','desc',0,5);
		getSchemeWiseLocationWiseAmountDetails(6,'villageLevlOvervw','scheme','count','desc',0,6);
			...
			table+='<table class="table table-striped table-condensed" id="dataTable'+divId+'" style="width:100%;">';
				...
				for(var i in result){
					...
					var lvlVal = 0;
						...
						table+='<td class="text-center">'+result[i].amount+'</td>';
						table+='<td class="text-center">'+result[i].subList[j].year+'</td>';
						var totalCount = 0;
						for(var k in result[i].subList[j].subList)
						{
							totalCount = totalCount + result[i].subList[j].subList[k].count
						}
						table+='<td class="text-center">'+totalCount+'</td>';
						for(var k in result[i].subList[j].subList)
						{
			...
			if(levelId == 2)
			{
			  if(result !=null && result.length>0){
				  ...
				  for(var i in result){
					...
					$(".chart"+divId).css("height","40 00px")
					$(".chart"+divId).height(height);*/
					$(".chart"+divId).highcharts({
					  chart: {
						type: 'column',
