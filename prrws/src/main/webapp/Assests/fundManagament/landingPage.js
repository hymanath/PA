...
function getHabitationCoverageByStatusByState()
{
 ...
 function getMeesevaSLAOverviewDtls(){
	 $(".itecMeeSevaSlaAllCls").html(spinner);
	...
	selectionMenu+='<ul class="menu-selection-body">';
		...
		for(var i in result)
		{
			selectionMenu+='<li>';
				...
				}else if(result[i].name == "GP Buildings1")
				{
				...
				}else if(result[i].name != "Greenary works (Chettu)" && result[i].name != "Agriculture Related Works" && result[i].name != "Rural Sanitation Works" && result[i].name != "Soil Moisture Conservation works (Neeru)" && result[i].name != "Works in community lands" && result[i].name != "OTHERS" && result[i].name != "Institutional Development Works" && result[i].name != "Road Works" && result[i].name != "Water Harvesting Structures (Neeru)" && result[i].name != "Fisheries work" && result[i].name != "AH-Live Stock Related works"){
					...
					//selectionMenu+='<label><i class="fa fa-star starcolorChange OthersMCCColor" title="click to add as favourite component." attr_url="MGNREGSDashboard?component=OthersMCC" attr_full_block_name="OthersMCC" style="margin-right:5px;" attr_color_name="gray" attr_block_name="OthersMCCColor" aria-hidden="true"></i>OthersMCC</label>';
				}
				/*else{
					selectionMenu+='<label><i class="fa fa-star starcolorChange '+result[i].name.replace(/\s+/g, '')+'Color" title="click to add as favourite component." attr_url="MGNREGSDashboard?component='+result[i].name+'" attr_full_block_name="'+result[i].name+'" style="margin-right:5px;" attr_color_name="gray" attr_block_name="'+result[i].name.replace(/\s+/g, '')+'" aria-hidden="true"></i>'+result[i].name+'</label>';
				}*/
...
function getSessionToken(){
	$(".waterTankClorinationAllCls").html(spinner);	
	var json = {
...
function getWaterBodyCumulativeCounts(sessionToken){
		...
		$.ajax({                
			...
			if(result !=null){
				$(".waterTankClorinationAllCls").html(result.clorinated);
			}else{
				$(".waterTankClorinationAllCls").html("0");
			}
