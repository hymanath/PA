	...
	$.ajax({
		...
		success: function(ajaxresp){
			if(ajaxresp != null && ajaxresp.length > 0){
				...
				var ONGOING = parseInt(ajaxresp[0].subList[0].undrProcessCount)+parseInt(ajaxresp[0].subList[1].undrProcessCount); 
				
				$(".RWSWORKSAllCls").html(notGrounded+' / '+ONGOING);
			}else{
...
function getLocationWiseWorksInformation(){
	...
	var json = {"locationType":"state","fromDateStr":"01-04-2014","toDateStr":"01-04-2050"}
	$.ajax({                
		type:'POST',    
		...
		if(result != null && result.length > 0){
			$(".EncWorksAllCls").html(''+result[0].notGrounded+' / <small style="font-size:14px;top:0px;font-weight:bold;">'+result[0].underProcessCount+'</small>');
		}else{
...
function getVehicletrackingDetails(){
	...
	$.ajax({                
		...
		if(result !=null){
			...
			$(".VehicleTrackingAllCls").html(offField+' / '+pending+' ( '+InProgress+' ) '); 
		}else{
