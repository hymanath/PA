...
function getUsers(constituencyId){
	...
	$(document).on('click','.issuesBtn',function(){
		...
		//}else{
			...
			//$("#userDescriptionId").html("<i>"+userName+" - "+userMobile+"</i>");		
		//}
	   $("#hdnCdrSrvyUserId").val(surveyUserId);
	   $("#hdnTabUserInfoId").val(tabUserId);
	...
	function getMembersDetails(surveyUserId,tabUserId,webUserId,userName,userMobile,minValue,resultType,status){
	   ...
	   $("#issuesDataMonitroingDashboardModal").modal('show');
		...
		$.ajax({
			...
			if(result != null && result.length > 0){  
				buildVerifiedDtlsCount(result,surveyUserId,tabUserId,0,userName,userMobile,minValue,resultType,verificationStatus);  
			}else{
				if(resultType=="All"){    
						...
						if(result[0][i].status == "noStatus"){
							...
							str+='<tr class="ownDeleteRow'+i+'">';    
								str+='<td>';  
									...
									str+='<button class="btn btn-sm btn-info changeImageBtnId" style="margin-left: 5px;" attr_cadre_id="'+result[0][i].cadreId+'" attr_dist_id="'+result[0][i].districtid+'" attr_const_id="'+result[0][i].constitunecyId+'" attr_cadre_survey_user_id="'+result[0][i].cadreUserId+'" attr_tab_user_id="'+result[0][i].tabUserId+'" attr_status="'+result[0][i].status+'" attr_divId="successMsgId'+i+'" attr_loading_img_id="changeImgLoadingImgId'+i+'">Change Image</button>';
					...
					for(var i in result[1]){ 
						...
						if(result[1][i].status == "Rejected"){
							...
							str2+='<tr>';
								...
								str2+='<button class="btn btn-xs btn-success updateImageBtnId"  style="margin-left: 141px; margin-top: -30px;" attr_position_id="'+i+'" attr_cadre_id="'+result[1][i].cadreId+'" attr_dist_id="'+result[1][i].districtid+'" attr_const_id="'+result[1][i].constitunecyId+'" attr_cadre_survey_user_id="'+result[1][i].cadreUserId+'" attr_tab_user_id="'+result[1][i].tabUserId+'" attr_status="'+result[1][i].status+'" attr_divId="successMsgRelId'+i+'" attr_loading_img_id="changeImgLoadingRelImgId'+i+'">Approve</button>';
