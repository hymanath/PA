...
function getNominatedPostApplication(startIndex)
	...
	if(searchRadioType == 'name')
		...
		var jsObj=
			{			
				searchType:searchRadioType,
				searchValue:searchValue
              				
		}
		$.ajax({
				type : "POST",
				url : "getUpdateNominationPostCadreSearchDetailsAction.action",
				data : {task:JSON.stringify(jsObj)} ,
			}).done(function(result){
			...
			{	
				...
				if(result[i].id != null && result[i].id >= 0){ // no cadre search  candidate id
					...
					str +='<img style="width: 70px;height:70px;border:1px solid #ddd;" src="https://mytdp.com/images/'+result[i].imageURL+'" class="img-responsive img-circle" alt="Profile"/>';
						str +='</div>';
					str +='<input type="checkbox" attr_cadreId="'+result[i].tdpCadreId+'" class="cadreCls checkboxCls hideShowDivCls" name="checkbox" style="margin:auto;display:block;" id="appProfCheckBoxId" sri attr_nominated_post_candidate_id="'+result[i].id+'" attr_membership_id="" />';
				}else {
			   ...
			   // str +='<input type="checkbox" style="margin:auto;display:block;" class="" />';
				...
				str +='<input type="hidden" class="tdpCadreIdCls" value="'+result[i].tdpCadreId+'" attr_nominated_post_candidate_id="'+result[i].id+'"/>';
				
				str +='<p class="text-center m_0"><b> ASSEMBLY : </b><span style="white-space: pre-wrap;"> '+result[i].casteName+'</span></p>';
						str +='</li>';
...
function getApplicationDocuments(cadreId,candiId,applicationId){
 ...
 $(document).on('click', '#advanceSearchBtnId', function(){
  ...
  function uploadDocuments(cadreId,candiId,applicationId){
   ...
   $("#hiddenApplicId").val(applicationId);
  }
  function saveCadrefileDetails(){
		//saveFlag=false;
		var uploadHandler = {
				upload: function(o) {
					$("#savingAjaxImg").css("display","none");
			...
			YAHOO.util.Connect.setForm('fileUploadForCadre',true);
			YAHOO.util.Connect.asyncRequest('POST','fileSavingDetailsAction.action',uploadHandler);
