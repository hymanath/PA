...
function showFirstFourNewsRecords(results)
 ...
 function showNewsWithDiffSources(fileId,filSize,arrayType)
 {
	...
	lb.innerHTML='<div class="fb-comments" data-href="https://www.partyanalyst.com/specialPageAction.action?specialPageId='+specialPageId+'&contentId='+results.contentId+'" data-num-posts="5" data-width="500"></div>';
	FB.XFBML.parse(lb);
	...
	if(results.length>0)
	 {
	  ...
	  {
	   count++;
	   str+='<li style="cursor:default;"><img class="imageClass" alt="'+results[0].title+'" src="'+results[0].path+'" onclick="getCandidatesPhotosInAGallary(\''+results[0].gallaryId+'\')" title="'+results[0].gallaryDescription+'"/><br />';
	  ...
	  {
	  count++;
	  str+='<li style="cursor:default;"><img class="imageClass" alt="'+results[1].title+'" src="'+results[1].path+'" onclick="getCandidatesPhotosInAGallary(\''+results[1].gallaryId+'\')" title="'+results[1].gallaryDescription+'"/><br />';
	  str+=''+results[1].title+'</li>';
	  }
	  if(results[2]!=null  && results[2].path!=null)
	  {
	  count++;
	  str+=' <li style="cursor:default;"><img class="imageClass" alt="'+results[2].title+'" title="'+results[2].gallaryDescription+'" src="'+results[2].path+'"  onclick="getCandidatesPhotosInAGallary('+results[2].gallaryId+')"/><br />';
	  ...
	  for(var i=3;i<results.length;i++)
	  {
	   ...
	   {
		count++;
		str+='<li style="cursor:default;"><img class="imageClass" alt="'+results[i].title+'" title="'+results[i].gallaryDescription+'" src="'+results[i].path+'"  onclick="getCandidatesPhotosInAGallary('+results[i].gallaryId+')"/><br />';
...
/*function buildVideoGallaries(results)
{
	...
	for(var i=0;i<results.length;i++)
	{
		...
		str += '<tr><td>';
		str+='<img src="https://img.youtube.com/vi/'+results[i].path+'/0.jpg" width="72px;" height="75px;" style="cursor: pointer;" onClick="getVideosInAGallary('+results[i].gallaryId+')"/></td></tr>';
		str+='</div>';
...
function buildVideoGallaries(results)
{
	...
	for(var i=0;i<results.length;i++)
	{
		if(i%4 == 0)
			...
			str +='<tr><td style="padding-left:4px;"><img src="https://img.youtube.com/vi/'+results[i].path+'/0.jpg" height="120px;" width="110px;" style="cursor: pointer; border-radius: 5px 5px 5px 5px; left: 13px; padding: 7px; border: 1px solid #ccc; background: none repeat scroll 0pt 0pt #d3d3d3;" onClick="getVideosInAGallary('+results[i].gallaryId+')"/>';
			str +='</td></tr>';
 ...
 var videosDivElmt = document.getElementById("videoGallaryPopUpDiv");
	...
	str+='<a href=javascript:{} style="text-decoration:none;color: #FFFFFF;margin-left: 339px;"" onclick="showAllVideoGalleries()" class="imageButton">Back To Galleries</a>';
		...
		for(var i in results)
		{
			...
			str+='<img src="https://img.youtube.com/vi/'+results[i].pathOfFile+'/0.jpg" width="160px;" height="160px;" title="'+results[i].description+'"/></a></td>';
			str+='</tr>';
			str+='<tr>';
