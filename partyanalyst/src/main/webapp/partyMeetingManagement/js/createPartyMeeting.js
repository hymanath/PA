...
function onLoadCalls(){
	...
	$('.endDateCls').datetimepicker({
	 ...
	 $(document).on("change","#meetingLevelId",function(){
		 $("#locationDivId").show();
		   //$('#mandalSpinnerId').show();
		 var meetingLevelId=$('#meetingLevelId').val();
...
"<label for='session'>Session:</label>"+ 
	 "<select class='form-control formIdFind sessionTypeId' id='sessionId' required >"+
       ...
       $('.ltTime').datetimepicker({
		...
		var jsObj = {
		    ...
		    for(var i in results){
		     ...
		     $(".formIdFind").append('<option value='+results[i].id+'>'+results[i].name +'</option>');
			}
