...
//getUserTypeWisePartyMeetingMOMDetails();
	...
	function getUserTypeWisePartyMeetingMOMDetails(){
		...
		$("#userTypeWisePartyMeetingMOMDetailsForTopFiveStrongAndPoorDiv").html('<div class="col-md-12 col-xs-12 col-sm-12"><div class="spinner"><div class="dot1"></div><div class="dot2"></div></div></div>');
	  ...
	  // alert(1111);
	   ...
	   if($("#tdpCommitteeYearId").val() != null)
		...
		$.ajax({
			...
			$("#userTypeWisePartyMeetingMOMDetailsForTopFiveStrongAndPoorDiv").html('');
			 buildgetUserTypeWisePartyMeetingMOMDetailsCountsForTopFiveStrongResults(result);
			 globalUserWiseMemberRslt = result;
		});
	}

function buildgetUserTypeWisePartyMeetingMOMDetailsCountsForTopFiveStrongResults(result){
		$("#userTypeWisePartyMeetingMOMDetailsForTopFiveStrongAndPoorDiv").html('');
		...
		if(result != null && result.length > 0){
			for(var i in result){
				...
				if(result[i] !=null && result[i].length>0){
					for(var j in result[i]){
						 ...
						 var obj1 = {
								...
								y: result[i][j].momCompletedCntPer
							};
						var obj2 = {
								name: result[i][j].name,
								y: result[i][j].totalPendingMomsCntPer
							};
						/*var obj3 = {
								name: result[i][j].name,
				...
				//if( result[i][j].momCompletedCntPer !=0 || result[i][j].totalPendingMomsCntPer !=0 || result[i][j].notStartedPerc !=0){
					...
					str+='</div>'
					$("#userTypeWisePartyMeetingMOMDetailsForTopFiveStrongAndPoorDiv").append(str);
					
					$(function () {
						 $("#genSec"+i).highcharts({
							 colors: ['#FF0000',' #145a32 '],
							chart: {
								type: 'column'
							},
							title: {
			...
			//$("#userTypeWisePartyMeetingMOMDetailsForTopFiveStrongAndPoorDiv").html("No Data Available");
			$("#userTypeWisePartyMeetingMOMDetailsForTopFiveStrongAndPoorDiv").html("");
		}
	}
