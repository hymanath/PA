					...
					str+='<tr>';
						...
						str+='<td>';
							...
							if(result[i].overAllPerc !=null && result[i].overAllPerc>0.0){
								...
								str+='<p class="text-capital"><input class="performanceRating" value="'+result[i].overAllPerc+'" type="hidden" class="rating" min=0 max=5 step=0.2 data-size="xs"  data-readonly><span class="label label-default label-xs labelCustom">'+result[i].overAllPerc+'</span></p>';
...
$(document).on("click",".designationWiseDebateCls",function(){
	var popupLocationId =[];
	$(".addClassClose").addClass("closeModalDeb");
...
//SMS Poll code
function getSMSPollDetails() 
{
	$("#smsPollDetailsDivId").html('<div class="spinner"><div class="dot1"></div><div class="dot2"></div></div>');
	var jsObj={
		startDate: customStartDate,
		endDate: customEndDate,	
	}
	$.ajax({
		type : 'POST',
		url : 'getSMSPollQuestionDetailsAction.action',
		dataType : 'json',
		data : {task:JSON.stringify(jsObj)} 
	}).done(function(result){
		buildSMSPollDetails(result);			
	});
} 
function buildSMSPollDetails(result) 
{ 
	var str=''; 
	str+='<div class="table-responsive scrollCls" style="padding:0px 10px;">';
		...
		str+='</table>';
	str+='</div>';
	$("#smsPollDetailsDivId").html(str);
	if(result.length> 4)
	{
		$(".scrollCls").mCustomScrollbar({setHeight:'200px'})
	} 
}