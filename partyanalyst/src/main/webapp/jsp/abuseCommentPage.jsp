...
function showAllNewPostedAbuseComment()
{
	...
	str += '<table>';
	str += '<tr>';
	str += '	<th>View Abused Comments between </th>';
...
{ 
	...
	if(!elmt)
     ...
     if(results.length >0)
     {
	    ...
	    str += '       <th style="color:#643918;width: 80px;">SELECT</th><th style="color:#643918; width: 80px;">COMMENT</th><th style="color:#643918; width: 80px;">USER</th><th style="color:#643918; width: 80px;">TIME</th><th style="color:#643918; width: 80px;">STATUS</th>';
        str += '     </tr>';
		for(i=0 ; i < results.length ; i++)
        {
			str += '<tr style="text-align:center">';
			str += '<td> <form><input type="checkbox" name="check"  value='+results[i].candidate+' /></form> </td>';
			//str += '<td><textarea cols="15" id="userMessageId'+i+'" value='+results[i].message+'>'+results[i].message+'</textarea></td>';
			str += '<td cols="15" id="userMessageId'+i+'" value='+results[i].message+'>'+results[i].message+'</td>';
			str += '<td>'+results[i].postedBY+'</td>';
			str += '<td>'+results[i].time+'</td>';
...
function controlSelectedReasons(task)
{
	...
	var checkedElmts = new Array();
	    ...
	    for(var i=0;i<elmts.length;i++){
	      if(elmts[i].checked == true )
		  {
			...
			{
			   if(elmts[i].value == myResult[j].candidate)
			   {
			        var message_id_obj={
					
					 updated_message : myResult[j].status,
   ...
   else
   {
	...
	var rparam ="task="+YAHOO.lang.JSON.stringify(jsObj);
	var url = "<%=request.getContextPath()%>/controlAbuseCommentsAction.action?"+rparam;					
	callAjax(jsObj,url);
}
...
<body>	
	<div id="commentsControlMain">		
		...
		<div id="reasonsAdminPageHeader"> Abused Comments Admin Control </div>
			...
			<div>
				<table width="100%" cellspacing="0" cellpadding="0" border="0">
					<tr>
						...
						<td><div class="headerLabelDiv_main"><span class="headerLabelSpan_main">Newely Abused Comment</span></div></td>
			...
			<div id="reasonsApprovediv" style="text-align:right;padding:10px;">
				<table width="100%">
					<tr>
						...
						<td width="20%">
							<input type="button" title="Click here to make comments are abused" class="buttonClass" value="Approve" onclick="controlSelectedReasons('approved')"></input>
							<input type="button" title="Click here to make comments are not abused" class="buttonClass" value="Reject" onclick="controlSelectedReasons('rejected')"></input>
						</td>
					</tr>
				</table>
