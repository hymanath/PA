					...
					<div class="row">
						...
						<div class="col-sm-4">
							<ul class="switch-btn pull-right">
								...
								<li class="active"> + New Grievance</li>
							</ul>
						</div>
					</div>
				</div>
								...
								<div class="panel-body bg_EF">
									<div class="row">
										...
										<div class="col-sm-3 m_top10" >
											...
											<select class="chosen" id="grievanceCategoryId" name="grievanceAlertVO.alertCategoryId">
										...
										<div class="col-sm-3 m_top10">
											...
											<select class="chosenSelect" id="departmentId" name="grievanceAlertVO.departmentId">  
											...
											<div class="row">
												...
												<div class="col-sm-3 m_top10">
													...
													<select class="chosen" id="alertlevelId" onChange="disableByGrievLevel();">
														...
														<option value="6">Village/Urban Locality</option>
														<option value="7">Habitation/Urban Block</option>
													</select>
												</div>
...
var globalLevelValue = 0;
var globalLevelId = 0;
var globalScopeLevelId = 0;
var globalScopeLevelValue = 0;
function getRegionScopesLevelValue(){
	...
	$.ajax({
		type : 'GET',
		url : 'getRegionScopeValuesForUserAction.action',
		dataType : 'json',
		data : {task:JSON.stringify(jobj)} 
	}).done(function(result){
		if(result != null){
			globalLevelValue = result.count;
			globalLevelId = result.id;
			globalScopeLevelId = result.totalCount;
			globalScopeLevelValue = result.scopeValue;
		}
	});
}
		...
		if(result != null && result.length > 0){    
			for(var i in result){
				if(result[i].id > 0 && result[i].id != 517)
					if(result[i].id == globalLevelValue)
...
function createGrievanceAlert()
{
  ...
  var categoryId = $("#grievanceCategoryId").val();
 ...
 if(level==4)
  ...
  if(department == 0){
	...
	//if(validate){
		...
		YAHOO.util.Connect.asyncRequest('POST','saveMeekosamGrievanceAlertAction.action',uploadHandler);
	//}
...
//getDepartmentLevels();
	...
	function getDepartmentLevels(){
		var depttId = $("#departmentId").val();
		var levelId = globalScopeLevelId;
		
		var jsObj = {
			departmentId : depttId,
			levelId : levelId
		}
		$.ajax({
		  type:'GET',
	...
	function getParentLevelsOfLevel(){
		...
		var levelValues = [];
		levelValues.push(globalScopeLevelValue);
		//departmentId = 49;
		var jsObj = {
			departmentId : departmentId,
			levelId : $("#locationLevelSelectId").val(),
			parentLevelId : globalScopeLevelId,
			parentLevelValueArr : levelValues
		}
		$.ajax({
		  type:'GET',
