...
str+='<span class="add-on"><i class="icon-envelope"></i></span><input type="text"  class="span2" class="input-small" placeholder="Email" name="userName"  id="userName1" style="height:22px !important;width: 200px; position: static;"/>';
str+='</div>';
str+='<div  class="input-prepend" style="margin-left: 55px;">';
str+='<span class="add-on"><i class="icon-lock"></i></span><input type="password"  class="span2" class="input-small" placeholder="Password" name="password" style="height:22px !important;width: 200px;position: static;" id="passWord_Id1"/>';
str+='</div>';
str+='<div class = "span3">';

str+='<input id="signin" class="submitButton btn btn btn-primary" onclick="ajaxCallForLoginPopup();" style="margin-left: 161px;  height: 25px; width: 85px; padding-top: 2px;" value="Sign In"/>';
str+='</div>';
...
function showLoginStatus(results)
{
  ...
  if(results.resultCode == 0)
  {
		...
		if(results.host == 'tdpserver' || results.host == 'localhost' || results.tempHost == 'tdpserver')
		  { 			
				if(tempStr.indexOf("govtLogin") != -1 && results.message == "PARTY_ANALYST_USER"){
