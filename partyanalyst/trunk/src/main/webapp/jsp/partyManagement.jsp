...
document.getElementById("profileManagementMainOuterDiv6").style.display = 'none';
    var jsObj =
		{   
			...
			startRecord:0,
			maxRecord:20,
			task:"getPartyPhotoGallaryDetail"
		};
...
function buildCandidatePhotoGallary(results)
{
	...
	else
	{
		...
		for(var i in results)
		{
			...
			{
			 str += '<tr><td><a href="javascript:{}" title="'+results[i].gallaryDescription+'"><img src="Thumbs/Medium/'+results[i].path+'" class="gallaryImg" onclick="getCompleteGallaries(\''+results[i].gallaryId+'\')"/></a></td></tr>';
			}
			else
			{
...
function getCompleteGallaries(gallaryId){
   ...
   for(var i in results)
   {
     ...
     str+= '<tr><td><a rel="photo_gallery" href="'+results[i].path+'" title="'+results[i].fileDescription1+'"><img alt="" src="Thumbs/Medium/'+results[i].path+'" class="gallaryImg" height="100px" /></a></td></tr>';
...
function formValidation()
{
	...
	var elmt1 = document.getElementById("partyId");
	...
	if(alltrim(elmt1.value) ==''){
		document.getElementById("alertMsg1").innerHTML ='<font color="red">Please Enter PartyId</font>';
...
/*function uploadVideoGallary()
{
	...
	var jsObj =
		{ 
			...
			fileDate	: fileDate,
			visibility	: makeThis,
		   	task		: "partyVideoGallariesForUplaod"
		};
	...
	function uploadPartyManifesto()
	  {
		...
		document.getElementById("profileManagementMainOuterDiv3").style.display = 'none';
		document.getElementById("profileManagementMainOuterDiv6").style.display = 'none';
		document.getElementById("profileManagementMainOuterDiv7").style.display = 'block';
		$("#photoGalleryId").css({"background":"none repeat scroll 0 0 #0063DC"});
		$("#videoGalleryId").css({"background":"none repeat scroll 0 0 #0063DC"});
		$("#newsGalleryId").css({"background":"none repeat scroll 0 0 #0063DC"});
		$("#developmentGalleryId").css({"background":"none repeat scroll 0 0 #0063DC"});
		$("#profileGalleryId").css({"background":"none repeat scroll 0 0 #0063DC"});
		 $("#manifestoId").css({"background":"none repeat scroll 0 0 #F61D50"});
  ...
  function uploadPartyManifestoDiv()
  {
	...
	function getElectionType()
	  ...
	  var jsObj =
		{ 
            time : timeST,
			task:"getElectionType"
		};

	var rparam ="task="+YAHOO.lang.JSON.stringify(jsObj);
	var url = "getElectionTypeAction.action?"+rparam;						
	callAjax(jsObj,url);
		...
		<tr>
          ...
          <tr>
			<td class="statusData_table_data" width="100%" style="padding-top:23px">
				<table>
				  <tr>
			    	<td style="padding-left:16px"><b><input type="button" class="buttonStyle" value="Photo Gallery" id="photoGalleryId" onClick="showPhotoGallary1()"></b></td>
