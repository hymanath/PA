<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<style type="text/css">
a:hover{
text-decoration:none;}
#uttarkhandExitPolltable > table * td {text-align:center;}
</style>
<div style="background:#ffffff; width: 979px;margin-left: 3px;padding-bottom: 13px;">
<img src="images/specialPage/uttarakhand2012election.png" style="align:center;width:982px;" usemap="#2012ElectionsMap" alt="Panjab 2012 Election" title="Panjab 2012 Election" />

<table width="100%" style="margin-left: 9px;margin-right: 9px; border-top: 1px solid #dddddd; margin-top: 4px;" ><tr><td  width="45%">

<table cellspacing="0" cellpadding="2" border="0" style="border: 1px solid #D2E888;width:100%">

<h3 style="padding:4px;background-color: #21B2ED;color:#ffffff;-moz-border-radius:3px;border-radius:3px;">2012 UttarKhand Exit Polls And Surveys</h3>

<tr><td>
<div id="uttarkhandExitPolltable" style="margin-top:3px;">
<span style="background:#D2E888;padding:4px;margin-left:9px;-moz-border-radius: 3px;">UttarKhand State Exit polls</span>
<table width="96%" cellspacing="0" cellpadding="5" bordercolor="#d2e888" border="1" style="border-collapse: collapse;margin-top:9px;margin-left:9px;">
<tr>


<th> Source</th>
<th>CONG</th>
<th>BJP</th>
<th>BSP</th>
<th>OTHERS</th>
</tr>
<tr>
    <th>NDTV</th>
   
    <td>34</td>
	<td>27</td>
    <td>3</td>
	<td>6</td>

  </tr>
  <tr>
 <th>STAR</th>
    <td>29(+8)</td>
    <td>36</td>
    <td>-</td>
	<td>5</td>
  </tr>
   <tr>
 <th>NEWS-24</th>
    <td>31-41</td>
    <td>22-32</td>
    <td>-</td>
	<td>-</td>
  </tr></tr>

</table>
</div>
</td></tr>


<tr><td colspan="7">
<BR />
<b><span style="-moz-border-radius:3px;padding:3px;margin-top: 15px;background: #d2e888;">2012 Uttarakhand Assembly Survey Results</span></b><br /><br />
<table bordercolor="#d2e888" cellspacing="0" cellpadding="5" border="1" align="center"  style="border-collapse: collapse; " width="96%">
            <tbody><tr>
			<td><b>Source</b></td>
              <td><b>INC</b></td>
              <td><b>BJP</b></td>
              <td><b>OTHERS</b></td>
            </tr>
			<tr><td>Star News</td><td>29</td><td>39</td><td>2</td></tr>
</table>
</td></tr>


<tr><td>
<table  bordercolor="#d2e888" cellspacing="0" cellpadding="5" border="1" align="center"  style="border-collapse: collapse;font-size:12px " width="96%">
            <tbody><tr style="background: #d2e888">
              <td width="64" class="TABLE_HEADER4">S. No.&nbsp;</td>
              <td width="130" class="TABLE_HEADER4">Poll Event&nbsp;</td>
              <td width="105" class="TABLE_HEADER4">Dates&nbsp;</td>
            </tr>
            <tr>
              <td align="center"><span class="paragraph1">1</span></td>
              <td><span class="paragraph1">Issue of Notification&nbsp;</span></td>
              <td><span class="paragraph1">05.01.2012 (Thursday)&nbsp;</span></td>
            </tr>
            <tr>
              <td align="center"><span class="paragraph1">2</span></td>
              <td><span class="paragraph1">Last date for making Nominations&nbsp;</span></td>
              <td><span class="paragraph1">12.01.2012 (Thursday)&nbsp;</span></td>
            </tr>
            <tr>
              <td align="center"><span class="paragraph1">3</span></td>
              <td><span class="paragraph1">Scrutiny of Nominations&nbsp;</span></td>
              <td><span class="paragraph1">13.01.2012 (Friday)&nbsp;</span></td>
            </tr>
            <tr>
              <td align="center"><span class="paragraph1">4</span></td>
              <td><span class="paragraph1">Last date for withdrawal of&nbsp;    candidature&nbsp;</span></td>
              <td><span class="paragraph1">16.01.2012 (Monday)&nbsp;</span></td>
            </tr>
            <tr>
              <td align="center"><span class="paragraph1">5</span></td>
              <td><span class="paragraph1">Date of Poll&nbsp;</span></td>
              <td><span class="paragraph1">30.01.2012 (Monday)&nbsp;</span></td>
            </tr>
            <tr>
              <td align="center"><span class="paragraph1">6</span></td>
              <td><span class="paragraph1">Counting of Votes&nbsp;</span></td>
              <td><span class="paragraph1">06.03.2012 (Sunday)&nbsp;</span></td>
            </tr>
            <tr>
              <td align="center"><span class="paragraph1">7</span></td>
              <td><span class="paragraph1">Date before which election process shall be completed&nbsp;</span></td>
              <td><span class="paragraph1">09.03.2012 (Friday)&nbsp;</span></td>
            </tr>
          </tbody></table></td></tr>
<tr><td>
  <table width="70%" align="center" border="0"><tr><td align="center">
<div class="pft-sec"> <img src="./images/new_homepage/pft.jpg" alt=""/>
<div class="clear"></div>
<p></p>
<span class="gray">Are you a</span>
<strong>Politician
<span class="orange">/</span>Political Party
<span class="orange">/</span>Media...</strong> Want to know how you can be benefited with 
<span class="orange">PartyAnalyst</span> 
<div class="clear"></div>
<div class="clickhere-button">
<a href="viewFeaturesAction.action">Click Here to Learn More...</a></div>
</div>
</td></tr>
</table>		  
</td></tr>
</table>
	</td>
	<td width="2%"></td>
	<td width="43%" valign="top">
<table style="border:1px solid #d2e888;padding-left:20px" width="98%" valign="top">
<h3 style="padding:4px;background-color: #21B2ED;color:#ffffff;-moz-border-radius:3px;border-radius:3px;width: 96.5%;">
Uttarakhand Assembly Live Election Results</h3>



<tr><td>
<table>
      <td>
	    <tr>
          <td style="padding-left: 47px;"><span style="font-weight:bold">Total Seats:70</span></td>
          <td style="margin-right: 2px; border-right-width: 3px; padding-right: 7px; padding-left: 53px;"><span style="font-weight:bold">Know Results/Leading:70</span></td>
      
       </tr>
   </table>
  </td>
</tr>

<tr>
	<td>
		<table cellspacing="0" cellpadding="5" bordercolor="#d2e888" border="1" style="border-collapse: collapse; margin-top: 9px; width: 100%; margin-left: -10px;">
			<tr><th>Party Name</th>
			
			<th>Participated Seats</th>
			<th>2007 Won Seats</th>
			<th>Won</th>
			</tr>
			<tr>
			<th><a href="partyPageAction.action?partyId=239">BSP</a></th>
			<td>69</td>
			<td>8</td>
			<td>3</td>
			</tr>
			<tr>
			<th><a href="partyPageAction.action?partyId=163">BJP</a></th>
			<td>69</td>
			<td>34</td>
			<td>31</td>
			</tr>
			<tr>
			<th><a href="partyPageAction.action?partyId=362">INC</a></th>
			<td>69</td>
			<td>21</td>
			<td>32</td>
			</tr>
			<tr>
			<th><a href="partyPageAction.action?partyId=1216">Uttarakhand Kranti Dal(P)</a></th>
			<td>69</td>
			<td>21</td>
			<td>1</td>
			</tr>
			
			<tr>
			<th><a href="javascript:{}">Others</a></th>
			<td>-</td>
			<td>-</td>
			<td>3</td>
			</tr>
			
		</table>
	</td></tr>

	<tr>
<td>


<table>
<tr>
 </td> 
   <center><object height="220" width="320"><param value="http://www.youtube.com/v/mMTRWXNVXCw?version=3&autoplay=1" name="movie"><param value="true" name="allowFullScreen"><param value="always" name="allowscriptaccess"><embed height="220" width="320" allowfullscreen="true" allowscriptaccess="always" type="application/x-shockwave-flash" src="http://www.youtube.com/v/mMTRWXNVXCw?version=3&autoplay=1"></object></center>
 </td>
</tr>
<tr>
 </td> 
<br>
<span style="-moz-border-radius:3px;padding:3px;background: #d2e888;font-weight:bold;">View Party Previous Performances</span>
<br><br>
</td>
</tr>
<tr>

<th style="padding-right: 27px;"><a href="partyResultsAction.action?selectedPartyShortName=BSP&selectedPartyId=239&selectedElectionTypeName=Assembly&selectedLocationName=Uttaranchal&electionType=2&reportLevel=State&stateSelectName=26&partySelectName=239&alliances=true&__checkbox_alliances=true&submitButton=Submit">BSP</a></th>
<th style="padding-right: 27px;"><a href="partyResultsAction.action?selectedPartyShortName=BJP&selectedPartyId=163&selectedElectionTypeName=Assembly&selectedLocationName=Uttaranchal&electionType=2&reportLevel=State&stateSelectName=26&partySelectName=163&alliances=true&__checkbox_alliances=true&submitButton=Submit">BJP</a></th>
<th style="padding-right: 27px;"><a href="partyResultsAction.action?selectedPartyShortName=INC&selectedPartyId=362&selectedElectionTypeName=Assembly&selectedLocationName=Uttaranchal&electionType=2&reportLevel=State&stateSelectName=26&partySelectName=362&alliances=true&__checkbox_alliances=true&submitButton=Submit">INC</a></th>
<th style="padding-right: 27px;"><a href="partyResultsAction.action?selectedPartyShortName=CPI&selectedPartyId=265&selectedElectionTypeName=Assembly&selectedLocationName=Uttaranchal&electionType=2&reportLevel=State&stateSelectName=26&partySelectName=265&alliances=true&__checkbox_alliances=true&submitButton=Submit">CPI</a></th>
<th style="padding-right: 27px;"><a href="partyResultsAction.action?selectedPartyShortName=CPM&selectedPartyId=269&selectedElectionTypeName=Assembly&selectedLocationName=Uttaranchal&electionType=2&reportLevel=State&stateSelectName=26&partySelectName=269&alliances=true&__checkbox_alliances=true&submitButton=Submit">CPM</a></th></tr>
</table>
</td>
</tr>

<tr><td colspan="7"> <br />
<b><span style="-moz-border-radius:3px;padding:3px;margin-top: 15px;background: #d2e888;">Previous Uttarakhand Assembly Elections Results </span></b><br /><br />
&nbsp;<img src="images/icons/diamond.png">
<a style="color: rgb(255, 255, 255); background-color: threeddarkshadow; font-weight: bold; padding: 3px;" href="electionDetailsReportAction.action?electionId=134&stateID=26&stateName=Uttaranchal&electionType=Assembly&electionTypeId=2&year=2007">2007</a></li>

&nbsp;<img src="images/icons/diamond.png">
<a style="color: rgb(255, 255, 255); background-color: threeddarkshadow; font-weight: bold; padding: 3px;" href="electionDetailsReportAction.action?electionId=135&stateID=26&stateName=Uttaranchal&electionType=Assembly&electionTypeId=2&year=2002">2002</a>

<tr style="margin-left: 10px; margin-top: 5px;">
<td colspan="7"><br><b><span style="-moz-border-radius:3px;padding:3px;margin-top: 15px;background: #d2e888;">Major Parties In the State</span></b><br /><br />
		   
<img src="images/icons/diamond.png">
<a href="partyPageAction.action?partyId=895">UKKD &nbsp;&nbsp;
<img src="images/icons/diamond.png"></a>
<a href="partyPageAction.action?partyId=163" align="top">BJP&nbsp;<img src="images/party_flags/BJP.png" height="40px" width="60px"></a>&nbsp;&nbsp;&nbsp; 
<img src="images/icons/diamond.png">
<a href="partyPageAction.action?partyId=362" align="top">INC&nbsp;<img src="images/party_flags/INC.png" height="40px" width="60px"></a>&nbsp;&nbsp;&nbsp; 
<img src="images/icons/diamond.png">
<a href="partyPageAction.action?partyId=239" align="top">BSP&nbsp;<img src="images/party_flags/BSP.png" height="40px" width="60px"></a>&nbsp;&nbsp;






</td></tr>

<!--<tr><td colspan="7">
<BR />
<b><span style="-moz-border-radius:3px;padding:3px;margin-top: 15px;background: #d2e888;">2012 Uttarakhand Assembly Survey Results</span></b><br /><br />
<table bordercolor="#d2e888" cellspacing="0" cellpadding="5" border="1" align="center"  style="border-collapse: collapse; " width="100%">
            <tbody><tr>
			<td><b>Source</b></td>
              <td><b>INC</b></td>
              <td><b>BJP</b></td>
              <td><b>OTHERS</b></td>
            </tr>
			<tr><td>Star News</td><td>29</td><td>39</td><td>2</td></tr>
</table>
</td></tr>-->


</table>



</td></tr>
</table>
<!--<div id="uttarkhandExitPolltable" style="margin-top:3px;">
<span style="background:#D2E888;padding:4px;margin-left:9px;-moz-border-radius: 3px;">UttarKhand State Exit polls</span>
<table width="47%" cellspacing="0" cellpadding="5" bordercolor="#d2e888" border="1" style="border-collapse: collapse;margin-top:9px;margin-left:9px;">
<tr>


<th> Source</th>
<th>CONG</th>
<th>BJP</th>
<th>BSP</th>
<th>OTHERS</th>
</tr>
<tr>
    <th>NDTV</th>
   
    <td>34</td>
	<td>27</td>
    <td>3</td>
	<td>6</td>

  </tr>
  <tr>
 <th>STAR</th>
    <td>29(+8)</td>
    <td>36</td>
    <td>-</td>
	<td>5</td>
  </tr>
   <tr>
 <th>NEWS-24</th>
    <td>31-41</td>
    <td>22-32</td>
    <td>-</td>
	<td>-</td>
  </tr></tr>

</table>
</div>-->

</div>
