...
/*function showImportantFamiliesDiv()
	...
	$('#constituencyResults').html('');
		...
		str+='<div id="cnstHeading" style="background:#f0f0f0;border-radius:0px;text-align:left;">Previous Election Results of '+cnstncy+' '+results[0].electionType+' Constituency <a id="ShowConstMenu" class="btn pull-right" style="margin-top:0px;" href="javascript:{}" >Hide<i class="icon-chevron-up"></i></a></div>';
...
function buildPartyWiseCastData(results,typeName,publicationDateId,boothId,type)
{
		  ...
		  if(Data == null || Data.length == 0)
		    return;
		  var str =' <table id="partyWiseJqTable" cellpadding="0" cellspacing="0" border="0" width="100%" style="border:3px solid black">';
          str+='  <thead>';
          str+='   <tr>';
          str+='     <th>Party</th>';
...
function buildPartyWisePiechart(myResults,jsObj)
{
   ...
   function buildLocalCastStatisticsDataTableForBooth(typeName,publicationDateId,boothId,type)
	...
	$("#localCastStatsVotersTitle").removeClass("localCastStatsVotersTitle");
         var str =' <table id="localCastStatsJqTable" cellpadding="0" cellspacing="0" border="0" width="100%" style="border:3px solid black">';
          str+='  <thead>';
          str+='   <tr>';
          str+='     <th>Caste</th>';
		  str+='	 <th>Caste category</th>';
		     ...
		     if(mandalid==mandals.id && mandals.selectOptionsList != null){
					  $.each(mandals.selectOptionsList,function(iter,panchayats){
							 if(panchayatid==panchayats.id && panchayats.selectOptionsList != null){
									...
									str+='<li class="nav nav-pills"><a class="btn" data-panchayatid="'+ panchayats.id+'" data-mandalid="'+ mandals.id+'" data-boothid="'+ booths.id+'" name-booth="Booth-'+booths.name+'" href="javascript:{}"><span class="checkbox"><input type="radio" data-mandalid="'+ mandals.id+'" id="Chk-'+booths.id+'" style="margin-top: -2px; margin-right: 4px;" name="menugroup" ></span>'+booths.name+'</a></li>';
									 });
								}
					 }); 
			}
