...
$("document").ready(function(){
	...
	$("#requestLink").click(function(){
		...
		var rparam ="task="+YAHOO.lang.JSON.stringify(jsObj);
		var url = "getReqMessagesForPublicProfileAction.action?"+rparam;						
		callAjax1(jsObj,url);
	...
	$(".messagesLink").click(function(){
		...
		var rparam ="task="+YAHOO.lang.JSON.stringify(jsObj);
		var url = "getAllRequestMessagesForPublicProfileAction.action?"+rparam;	
	...
	//subscriptions
	 ...
	 $(".subscriptionsLink").click(function(){
		...
		var rparam ="task="+YAHOO.lang.JSON.stringify(jsObj);
		var url = "getUserSubScriptionsForPublicProfileAction.action?"+rparam;	
	...
	var rparam ="task="+YAHOO.lang.JSON.stringify(jsObj);	
	var url = "getAllPostedReasonsForPublicProfileAction.action?"+rparam+"&type="+type+"&sort=candidate&dir=asc";	
	...
	var rparam ="task="+YAHOO.lang.JSON.stringify(jsObj);	
	var url = "getAllPostedProblemsForPublicProfileAction.action?"+rparam+"&type="+type+"&sort=problemId&dir=desc";
...
function showAllPostedReasons_paginator(jsObj,results)
{
	...
	for(var i in results.candidateComments)
	{
		var template = $('.templateDivMsg');
		var templateClone = template.clone();
		templateClone.removeClass('templateDivMsg');
		//templateClone.find('.imgClass').html('message');
		templateClone.find('.imgClass').html(''+results.candidateComments[i].commentCategory+'');
		templateClone.appendTo('.placeholderCenterDiv');
	}
...
function showAllUserSubScribedSpecialPagesPages(jsObj,results)
{
	...
	if(results.profileSpecialPageSubscriptions == null || results.profileSpecialPageSubscriptions.length == 0)
	{
	...
	for(var i in results.profileSpecialPageSubscriptions)
	{
		var template = $(".specialPagSubscrTemplDiv");
		var templateClone = template.clone();
		templateClone.removeClass('specialPagSubscrTemplDiv');
		templateClone.find('.titleCls').html(''+results.profileSpecialPageSubscriptions[i].title+'');
		templateClone.find('.imgClass').html('<img height="100" width="95" src="'+results.profileSpecialPageSubscriptions[i].imageURL+'"/>');
		if(buildSubscribeButtons(results.userSpecialPageSubscriptions[i].id,results.userSpecialPageSubscriptions))
		...
		templateClone.find('.hiddenVar').html('<input type="hidden" value="'+results.userSpecialPageSubscriptions[i].id+'" class="hiddenVarId" /><input type="hidden" class="subscripType" value="specialPage"/>');
...
function showAllUserCandidateSubscriptions(jsObj,results)
{
	...
	if(results.profileCandidateSubscriptions == null || results.profileCandidateSubscriptions.length == 0)
	{
	...
	for(var i in results.profileCandidateSubscriptions)
	{
		var template = $(".specialPagSubscrTemplDiv");
		var templateClone = template.clone();
		templateClone.removeClass('specialPagSubscrTemplDiv');
		templateClone.find('.titleCls').html(''+results.profileCandidateSubscriptions[i].name+'');
		templateClone.find('.imgClass').html('<img height="100" width="95" src="images/candidates/'+results.profileCandidateSubscriptions[i].name+'.png"/>');
		if(buildSubscribeButtons(results.profileCandidateSubscriptions[i].id,results.userCandidateSubscriptions))
			templateClone.find('.btnClass').html('<a href="javascript:{}" class="unSubscribedLink">Un Subscribed</a>');
		else
			templateClone.find('.btnClass').html('<a href="javascript:{}" class="subscribedLink">Subscribed</a>');
		templateClone.find('.hiddenVar').html('<input type="hidden" value="'+results.profileCandidateSubscriptions[i].id+'" class="hiddenVarId" />');
		templateClone.find('.hiddenVar').html('<input type="hidden" value="'+results.profileCandidateSubscriptions[i].id+'" class="hiddenVarId" /><input type="hidden" class="subscripType" value="candidatePage"/>');
...
function showAllUserConstituencySubscriptions(jsObj,results)
{
	...
	if(results.profileConstituencySubscriptions == null || results.profileConstituencySubscriptions.length == 0)
	{
	...
	for(var i in results.profileConstituencySubscriptions)
	{
		var template = $(".specialPagSubscrTemplDiv");
		var templateClone = template.clone();
		templateClone.removeClass('specialPagSubscrTemplDiv');
		templateClone.find('.titleCls').html(''+results.profileConstituencySubscriptions[i].name+'');
		...
		if(buildSubscribeButtons(results.profileConstituencySubscriptions[i].id,results.userConstituencySubscriptions))
			...
			templateClone.find('.btnClass').html('<a href="javascript:{}" class="subscribedLink">Subscribed</a>');
		templateClone.find('.hiddenVar').html('<input type="hidden" value="'+results.profileConstituencySubscriptions[i].id+'" class="hiddenVarId" /><input type="hidden" class="subscripType" value="constituencyPage"/>');
...
function showAllUserPartySubscriptions(jsObj,results)
{
	...
	if(results.profilePartySubscriptions == null || results.profilePartySubscriptions.length == 0)
	{
	...
	for(var i in results.profilePartySubscriptions)
	{
		var template = $(".specialPagSubscrTemplDiv");
		var templateClone = template.clone();
		templateClone.removeClass('specialPagSubscrTemplDiv');
		templateClone.find('.titleCls').html(''+results.profilePartySubscriptions[i].title+'');
		templateClone.find('.imgClass').html('<img height="100" width="95" src="images/party_flags/'+results.profilePartySubscriptions[i].name+'.png"/>');
		if(buildSubscribeButtons(results.profilePartySubscriptions[i].id,results.userPartySubscriptions))
			templateClone.find('.btnClass').html('<a href="javascript:{}" class="unSubscribedLink">Un Subscribed</a>');
		else
			templateClone.find('.btnClass').html('<a href="javascript:{}" class="subscribedLink">Subscribed</a>');
		templateClone.find('.hiddenVar').html('<input type="hidden" value="'+results.profilePartySubscriptions[i].id+'" class="hiddenVarId" /><input type="hidden" class="subscripType" value="partyPage"/>');
