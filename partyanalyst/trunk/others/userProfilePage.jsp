	...
	<div class="container m-top15">
		...
		<!--------left div------->
			<div class="span3 left-section">
				...
				<div class="widget-block">
					...
					<li><a href="javascript:{}" onclick="getNews('byTodayDate','getCount','','','','','','','')" id="customerNews"><i class="icon-file"></i><i class="icon-chevron-right"></i> News</a></li>
					</c:if>
		...
		<!--------Center div------>
			<div class="span6 centerSpan6">
			    <div class="widget yellow" id="advancedDashBoard" style="display:none;">
	   ...
	   success : function( o ) {
			try {												
				myResults = YAHOO.lang.JSON.parse(o.responseText);	
					 ...
					 }else if(jsObj.task == "getAdvancedDashBoard"){
					   //buildAdvancedDashBoard(myResults);
					 }
...
function buildAdvancedDashBoard(myResult){
   var str ='';
   if(myResult != null && myResult.divReq != null && myResult.divReq == true){
             str+='<h2>ADVANCED DASHBOARD</h2>';
				 str+='<table class="table table-hover">';
				  str+='  <tr><td><a href="votersAnalysisNewAction.action?constituencyId='+myResult.assemblyId+'">Voters Analysis</a></td></tr>';
				  str+='  <tr><td><a href="votersSearchAction.action">Voters Search And Edit</a></td></tr>';
				  str+='  <tr><td><a href="crossVotingReportInputAction.action?partyId=872&electionYear='+myResult.asselectionYear+'&constituencyId='+myResult.assemblyId+'&parlconstituencyId='+myResult.parliamentId+'">Cross Voting Repory</a></td></tr>';
				  if(myResult.constituencyType == 'assembly')
				   str+='  <tr><td><a href="partyBoothResult2Action.action?partyName=872&electionYear='+myResult.asselectionYear+'&constituencyName='+myResult.assemblyId+'">Constituency Booth Results Report</a></td></tr>';
				  else if(myResult.constituencyType == 'parliament')
				   str+='  <tr><td><a href="partyBoothResult2Action.action?partyName=872&electionYear='+myResult.parlelectionYear+'&constituencyName='+myResult.parliamentId+'">Constituency Booth Results Report</a></td></tr>';
				  str+='</table>';
				 $("#advancedDashBoard").html(str);
...
function getAdvancedDashBoard(){
        var jsObj=
			{					
				...
				task:"getAdvancedDashBoard"
			}
		
			var rparam ="task="+YAHOO.lang.JSON.stringify(jsObj);
			var url = "getAdvanceDashBoardInfo.action?"+rparam;						
		callAjaxForUserSettings(jsObj,url);
}
