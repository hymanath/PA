...
public class ProblemManagementAction extends ActionSupport implements ServletRequestAware{
	...
	public String getProblemsByFilterView()
	{
		...
		Long statusId = null;
		try {
			sDate = jObj.getString("startText");
			eDate = jObj.getString("endText");
			statusId = jObj.getLong("statusValue");
	...
	public String getProblemsPostedByUserToBuildCharts(){
		...
		Long problemStatusId= null;
		String problemsBy = "";
		int startIndex = 0;
		int maxResults = 0;
		try {
			problemStatusId = jObj.getLong("statusId");
			String problemStatus = jObj.getString("statusName");
			problemsBy = jObj.getString("dateType");
			startIndex = jObj.getInt("startIndex");
			maxResults = jObj.getInt("maxResults");
		...
		if(problemStatusId == null || problemStatusId.equals(0L))
			problemManagementChartVO = problemManagementService.getProblemsDataForAUserToBuildChart(user.getRegistrationID(), startIndex, maxResults, getProblemsBy);
		else
			problemManagementChartVO = problemManagementService.getProblemsDataForAUserToBuildChart(user.getRegistrationID(), problemStatusId, startIndex, maxResults, getProblemsBy);
