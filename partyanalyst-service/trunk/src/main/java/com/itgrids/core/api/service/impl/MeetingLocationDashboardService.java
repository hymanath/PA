...
public class MeetingLocationDashboardService implements IMeetingLocationDashboardService{
	...
	public PartyMeetingDataVO getLocationWiseStateMeetings(Long locationTypeId, List<Long> locationValues,String fromDateStr,String toDateStr,Long partyMeetingMainTypeId){
		...
		try{
			...
			List<Object[]> attendees = partyMeetingAttendanceDAO.getLocationWiseStateMeetingAttendees(locationValues,locationTypeId,startDate,endDate,partyMeetingMainTypeId,null,null,null,null);
	...
	*/
	public PartyMeetingDataVO getLocationWiseSpecialMeetingsMeetingsExpanction(Long locationTypeId, List<Long> locationValues,String fromDateStr,String toDateStr,Long partyMeetingMainTypeId,Long partyMeetingTypeId){
		...
		try{
			...
			List<Object[]> attendees = partyMeetingAttendanceDAO.getLocationWiseStateMeetingAttendees(locationValues,locationTypeId,startDate,endDate,partyMeetingMainTypeId,partyMeetingTypeId,null,null,null);
				...
				for (Entry<Long, Map<Long, PartyMeetingDataVO>> entry :meetingTypeMap.entrySet()) {
					...
					if(commonMethodsUtilService.isMapValid(cadresmap)){
						for (Entry<Long, PartyMeetingDataVO> candidates :cadresmap.entrySet()) {
							...
							Set<Long> attendedIds = vo.getAttendedIds();
								if(commonMethodsUtilService.isListOrSetValid(attendedIds)){
									//meetingTypeVO.getAttendedIds().addAll(attendedIds);
									//meetingTypeVO.setAttendedCount(Long.valueOf(String.valueOf(meetingTypeVO.getAttendedIds().size())));
									for (Long tdpCadreId : attendedIds) {
										if(inviteeIds.contains(tdpCadreId)){
											//meetingTypeVO.getInviteeAttendedIdsList().add(tdpCadreId);
											//meetingTypeVO.setInviteeAttendedCount(Long.valueOf(String.valueOf(meetingTypeVO.getInviteeAttendedIdsList().size())));
								...
								if(commonMethodsUtilService.isListOrSetValid(inviteeIds)){
									...
									if(partyMeetingVO != null){
										...
										meetingTypeVO.setLateCount(meetingTypeVO.getLateCount()+partyMeetingVO.getLateCount());
										partyMeetingVO.getAttendedIds().addAll(attendedIds);
										meetingTypeVO.setAttendedCount(meetingTypeVO.getAttendedCount()+Long.valueOf(String.valueOf(partyMeetingVO.getAttendedIds().size())));
											...
											if(commonMethodsUtilService.isListOrSetValid(sessionAttendedIds)){
												...
												for (Long tdpCadreId : sessionAttendedIds) {
													if(inviteeIds.contains(tdpCadreId)){
														sessionVO.getInviteeAttendedIdsList().add(tdpCadreId);
														sessionVO.setRecentInviteeAttended(Long.valueOf(String.valueOf(sessionVO.getInviteeAttendedIdsList().size())));
														partyMeetingVO.getInviteeAttendedIdsList().add(tdpCadreId);
														sessionVO.setLatePerc(calculatePercantage(sessionVO.getRecentLate(),partyMeetingVO.getRecentMeetingInviteesCnt()).toString());
													}
												}
										...
										meetingTypeVO.setInviteeAttendedCount(meetingTypeVO.getInviteeAttendedCount()+Long.valueOf(String.valueOf(partyMeetingVO.getInviteeAttendedIdsList().size())));
									}
								}
						}
					}
							...
							PartyMeetingDataVO vo =cadresmap.get(commonMethodsUtilService.getLongValueForObject(param[5]));
								...
								if(partyMeetingVO != null){
									...
									if(type != null && type.equalsIgnoreCase("attendee")){
										...
										sessionVO.setRecentImagesCnt(sessionVO.getRecentImagesCnt().longValue()+1l);
									}
								  }
								}
					}
	...
	*  @return :void
	*/
	public List<IdNameVO> getLocationWiseMeetingInviteeMembers(Long locationTypeId, List<Long> locationValues,String fromDateStr,String toDateStr,Long partyMeetingMainTypeId,
			Long partyMeetingTypeId,Long partyMeetingId,String pastMeetings,Long sessionTypeId,String status){
		...
		try{
			...
			List<Object[]> attendees = partyMeetingAttendanceDAO.getLocationWiseStateMeetingAttendees(locationValues,locationTypeId,startDate,endDate,partyMeetingMainTypeId,partyMeetingTypeId,null,inviteesMap.keySet(),sessionTypeId);
			...
			if(commonMethodsUtilService.isListOrSetValid(invitees)){
				for(Object[] param :invitees){
					...
					if(inviteeMem == null ){
						...
						inviteeMem.setSubList1(setPartyMeetingsTemplate(attendees));
			...
			if(commonMethodsUtilService.isListOrSetValid(invitees)){
				for(Object[] param :invitees){
					...
					if(inviteeMem != null ){
						...
						if(inviteeMem.getInviteeAttnd().longValue() > 0l && (inviteeMem.getActualCount().longValue() == 0l || inviteeMem.getActualCount() == null)){
							inviteeMem.setAvailableCount(inviteeMem.getInviteeAttnd().longValue());
						}
			...
			if(commonMethodsUtilService.isMapValid(inviteesMap)){
				for(Map.Entry<Long, IdNameVO> entry :inviteesMap.entrySet()){
					IdNameVO inviteeMem =entry.getValue();
					if(status != null && status.equalsIgnoreCase("attendee")){
						if(inviteeMem.getActualCount() != null && inviteeMem.getActualCount().longValue() >0l){
							returnList.add(inviteeMem);
						}
					}else if(status != null && status.equalsIgnoreCase("abscent")){
						if(inviteeMem.getAvailableCount() != null && inviteeMem.getAvailableCount().longValue() >0l){
							returnList.add(inviteeMem);
						}	
					}else if(status != null && status.equalsIgnoreCase("invitee")){
						returnList.add(inviteeMem);
					}
				}
	...
	public List<IdNameVO> setPartyMeetingsTemplate(List<Object[]> attendees){
		...
		try{
			...
			e.printStackTrace();
			LOG.error("Exception raised at setPartyMeetingsTemplate", e);
		}
