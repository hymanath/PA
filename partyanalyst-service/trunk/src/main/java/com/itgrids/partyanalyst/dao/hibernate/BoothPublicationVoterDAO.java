...
public List<Object[]> getLatestBoothDetailsOfConstituency(Long constituencyId)
{
	...
	{
		Query query = getSession().createQuery("select model.booth.constituency.constituencyId,model.booth.constituency.name,count(distinct model.voter.voterId) from BoothPublicationVoter model where model.booth.publicationDate.publicationDateId = :publicationId and model.booth.constituency.constituencyId in( :constituencyIds) group by model.booth.constituency.constituencyId");
