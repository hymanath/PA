...
public class CadreDAO extends GenericDaoHibernate<Cadre, Long> implements ICadreDAO{
	...
	public List<Cadre> findByUserID(Long userID){
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model where model.user.userId=?", userID);
		return results;
	}
	...
	public List findCadreSizeStateWise(Long userID){
		List  results = getHibernateTemplate().find("Select model.currentAddress.state.stateId, count(model.currentAddress.state.stateId)from Cadre model " +
				"where model.user.userId = ? group by model.currentAddress.state.stateId", userID); 
		return results;
	}
	...
	public List findCadreSizeLocalElectionBodywise(Long userId) {
		List  results = getHibernateTemplate().find("Select model.currentAddress.localElectionBody.localElectionBodyId, count(model.currentAddress.localElectionBody.localElectionBodyId)from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.localElectionBody.localElectionBodyId is not null group by model.currentAddress.localElectionBody.localElectionBodyId", userId); 
		return results;
	}
	...
	public List findCadreSizeVillageWise(Long userID){
		List  results = getHibernateTemplate().find("Select model.village.townshipId, count(model.village.townshipId)from Cadre model " +
					"where model.user.userId = ? group by model.village.townshipId", userID); 
		return results;
	...
	public List findMandalCadresByConstituency(Long constituencyID, Long userID, String cadreType){
		...
		List  results = getHibernateTemplate().find("Select model.currentAddress.tehsil.tehsilId, model.currentAddress.tehsil.tehsilName, count(model.currentAddress.tehsil.tehsilId)from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.constituency.constituencyId=? and model.memberType = ? group by model.currentAddress.tehsil.tehsilId order by model.currentAddress.tehsil.tehsilName", params); 
		return results;
	}
	...
	public List findMandalCadresByDist(Long districtID, Long userID, String cadreType){
		...
		List  results = getHibernateTemplate().find("Select model.tehsil.tehsilId, model.tehsil.tehsilName, count(model.tehsil.tehsilId)from Cadre model " +
				"where model.user.userId = ? and model.district.districtId=? and model.memberType = ? group by model.tehsil.tehsilId order by model.tehsil.tehsilName", params); 
		return results;
	}
	@SuppressWarnings("unchecked")
	public List findVillageCadresByMandal(Long mandalID, Long userID, String cadreType){
		...
		List  results = getHibernateTemplate().find("Select model.currentAddress.hamlet.hamletId, model.currentAddress.hamlet.hamletName, count(model.currentAddress.hamlet.hamletId) from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.tehsil.tehsilId=? and model.memberType = ? group by model.currentAddress.hamlet.hamletId order by model.currentAddress.hamlet.hamletName", params); 
		return results;
	}
	...
	public List getUserAccessStates(Long userID) {
		List results = getHibernateTemplate().find("select distinct model.state.stateId, model.state.stateName from Cadre model " +
				"where model.user.userId = ? order by model.state.stateName", userID);
		return results;
	}
	...
	public List getUserAccessDistricts(Long userID) {
		List results = getHibernateTemplate().find("select distinct model.district.districtId, model.district.districtName from Cadre model " +
				"where model.user.userId = ? order by model.district.districtName", userID);
		return results;
	}
	...
	public List getUserAccessMLAConstituencies(Long userID) {
		List results = getHibernateTemplate().find("select distinct model.constituency.constituencyId, model.constituency.name from Cadre model " +
				"where model.user.userId = ? and model.constituency.electionScope.electionType.electionType='Assembly' order by model.constituency.name", userID);
		return results;
	}
	...
	public List getUserAccessMandals(Long userID) {
		List results = getHibernateTemplate().find("select distinct model.tehsil.tehsilId, model.tehsil.tehsilName from Cadre model " +
				"where model.user.userId = ? order by model.tehsil.tehsilName", userID);
		return results;
	}
	...
	public List getMobileNosByState(Long userID, Long stateID) {
		...
		List results = getHibernateTemplate().find("select  model.mobile, model.firstName, model.lastName from Cadre model " +
				"where model.user.userId = ? and model.state.stateId=?", ids);
		return results;
	}
	...
	public List getMobileNosByDistrict(Long userID, Long districtID) {
		...
		List results = getHibernateTemplate().find("select  model.mobile, model.firstName, model.lastName from Cadre model " +
				"where model.user.userId = ? and model.district.districtId=?", ids);
		return results;
	}
	...
	public List getMobileNosByConstituency(Long userID, Long constituencyId) {
		...
		List results = getHibernateTemplate().find("select  model.mobile, model.firstName, model.lastName from Cadre model " +
				"where model.user.userId = ? and model.constituency.constituencyId=?", ids);
		return results;
	}
	...
	public List getMobileNosByMandal(Long userID, Long mandalID) {
		...
		List results = getHibernateTemplate().find("select  model.mobile, model.firstName, model.lastName from Cadre model " +
				"where model.user.userId = ? and model.tehsil.tehsilId=?", ids);
		return results;
	}
	...
	public List getMobileNosByVillage(Long userID, Long villageID) {
		...
		List results = getHibernateTemplate().find("select  model.mobile, model.firstName, model.lastName from Cadre model " +
				"where model.user.userId = ? and model.village.townshipId=?", ids);
		return results;
	}
	...
	public List getMobileNosByHamlet(Long userID, Long hamletID) {
		...
		List results = getHibernateTemplate().find("select  model.mobile, model.firstName, model.lastName from Cadre model " +
				"where model.user.userId = ? and model.hamlet.hamletId=?", ids);
		return results;
	}
	...
	public List getMobileNosByCadreLevel(Long userID, Long level) {
		...
		List results = getHibernateTemplate().find("select  model.mobile, model.firstName, model.lastName from Cadre model " +
				"where model.user.userId = ? and model.cadreLevel.cadreLevelID=?", ids);
		return results;
	}
	...
	public List<Cadre> findCadresByDistrict(Long districtID, Long userID){
		...
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.district.districtId=?", params); 
		return results;
	}
	@SuppressWarnings("unchecked")
	public List<Cadre> findCadresByConstituency(Long constituencyID, Long userID){
		...
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.constituency.constituencyId=?", params); 
		return results;
	}
	...
	public List<Cadre> findCadresByMandal(Long mandalID, Long userID){
		...
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.tehsil.tehsilId=?", params); 
		return results;
	}
	...
	public List<Cadre> findCadresByState(Long stateID, Long userID){
		...
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.state.stateId = ?", params); 
		return results;
	}
	...
	public List getCadreSizeByHamlet(String revenueVillageIDs, Long userID, String cadreType){
		...
		List  results = getHibernateTemplate().find("Select model.currentAddress.hamlet.hamletId, model.currentAddress.hamlet.hamletName, " +
				...
				"where model.user.userId = ?  and model.currentAddress.township.townshipId in (" + revenueVillageIDs +") and model.memberType = ?"+
	...
	public List findCadreDetailsByLevelAndProperty(Long userId,
		...
		return getHibernateTemplate().find("select model.cadreId from Cadre model where model.user.userId = ?"+
	...
	public List findCadreByPropertyValueListAndUser(Long userId,
		...
		Query queryObject = getSession().createQuery("select model.cadreId from Cadre model where model.user.userId = ?"+
	...
	public List findConstituencyCadresByParliamentConst(Long districtID, Long userID, String cadreType){
		Object[] params = {userID,districtID, cadreType};
		List  results = getHibernateTemplate().find("Select model.currentAddress.constituency.constituencyId, model.currentAddress.constituency.name, count(model.currentAddress.constituency.constituencyId)from Cadre model " +
				" where model.user.userId = ? and model.currentAddress.parliamentConstituency.constituencyId=? and model.memberType = ? group by model.currentAddress.constituency.constituencyId order by model.currentAddress.constituency.name", params); 
		return results;
	}
	...
	public List findTownshipCadresByMandal(Long mandalID, Long userID,
		...
		List  results = getHibernateTemplate().find(" Select model.currentAddress.township.townshipId, model.currentAddress.township.townshipName, count(model.currentAddress.township.townshipId) from Cadre model " +
				" where model.currentAddress.tehsil.tehsilId=? and model.user.userId = ? and model.memberType = ? group by model.currentAddress.township.townshipId order by model.currentAddress.township.townshipName", params); 
		return results;
	...
	public List findCadresByWard(Long localElectionBodyId, Long userID,
		...
		List  results = getHibernateTemplate().find("Select model.currentAddress.ward, count(model.currentAddress.ward.constituencyId) from Cadre model " +
				"where model.currentAddress.localElectionBody.localElectionBodyId=? and model.user.userId = ? and model.memberType = ? group by model.currentAddress.ward.constituencyId order by model.currentAddress.ward.name", params); 
		return results;
	}
	...
	public List findCadresByBoothInWard(Long wardId, Long userId,
		...
		List  results = getHibernateTemplate().find("Select model.currentAddress.booth.boothId, model.currentAddress.booth.partNo, model.currentAddress.booth.location,  count(model.currentAddress.booth.boothId) from Cadre model " +
				"where model.currentAddress.ward.constituencyId=? and model.user.userId = ? and model.memberType = ? group by model.currentAddress.booth.boothId", params); 
		return results;
	}
	...
	public List<Cadre> findCadresByTownship(Long townshipId, Long userID, String cadreType){
		...
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.township.townshipId = ? and model.memberType = ?", params); 
		return results;
	}
	...
	public List<Cadre> getCadresByWard(Long wardId, Long userID, String cadreType) {
		...
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.user.userId = ? and model.currentAddress.ward.constituencyId=? and model.memberType = ?", params); 
		return results;
	}
	...
	public List findStateCadresByCountry(Long countryID, Long userID, String cadreType){
		...
		List  results = getHibernateTemplate().find("Select model.currentAddress.state.stateId, model.currentAddress.state.stateName, count(model.currentAddress.state.stateId)from Cadre model " +
				"where model.user.userId = ? and model.memberType = ? group by model.currentAddress.state.stateId order by model.currentAddress.state.stateName", params); 
		return results;
	}
	...
	public List findBoothCadresByLocalElectionBody(Long localElectionBodyId,
		...
		Object[] params = {localElectionBodyId,userID, cadreType};
		List  results = getHibernateTemplate().find(" Select model.currentAddress.booth.boothId, model.currentAddress.booth.partNo, model.currentAddress.booth.location, count(model.currentAddress.booth.boothId) from Cadre model " +
				" where model.currentAddress.localElectionBody.localElectionBodyId=? and model.user.userId = ? and model.memberType = ? and model.currentAddress.localElectionBody.localElectionBodyId is not null group by model.currentAddress.booth.boothId", params); 
		return results;
			}
	...
	public List<Cadre> findCadresByBooth(Long boothId, Long userID,
		...
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.currentAddress.booth.boothId=? and model.user.userId = ? and model.memberType = ? and model.currentAddress.booth.boothId is not null", params); 
		return results;
	}
	...
	public List<Cadre> findCadreDetailsNotAssignedToBoothInLocalBody(
		...
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.currentAddress.localElectionBody.localElectionBodyId = ? and model.user.userId = ? and model.memberType = ? " +
	...
	public List<Cadre> findCadreDetailsNotAssignedToBoothInWard(Long wardId, Long userId, String cadreType) {
		Object[] params = {wardId,userId,cadreType};
		List<Cadre>  results = getHibernateTemplate().find("from Cadre model " +
				"where model.currentAddress.ward.constituencyId = ? and model.user.userId = ? and model.memberType = ? " +
	...
	public List findCadresNotAssignedToBooth(Long userId, String cadreType) {
		...
		return getHibernateTemplate().find("select count(model.cadreId) from Cadre model where model.currentAddress.booth.boothId is null" +
				" and model.user.userId = ? and model.memberType = ?", params);
	}
	...
	public List findVillageLevelCadresContByMandal(Long mandalId, Long userId,
		...
		return getHibernateTemplate().find("select count(model.cadreId) from Cadre model where model.currentAddress.tehsil.tehsilId is not null" +
				" and model.tehsil.tehsilId = ? and model.user.userId = ? and model.memberType = ?", params);
	}
	...
	public List findLocalBodyLevelCadresContByMandal(Long localBodyId,
		...
		return getHibernateTemplate().find("select count(model.cadreId) from Cadre model where model.currentAddress.localElectionBody.localElectionBodyId is not null" +
				" and model.localElectionBody.localElectionBodyId = ? and model.user.userId = ? and model.memberType = ? group by model.currentAddress.localElectionBody.localElectionBodyId", params);
	}
	...
	public List findCadresNotAssignedToBoothLocalElectionBody(Long localBodyId,
		...
		Object[] params = {localBodyId, userId,cadreType};
		return getHibernateTemplate().find("select count(model.cadreId) from Cadre model where model.currentAddress.booth.boothId is null" +
				" and model.currentAddress.localElectionBody.localElectionBodyId = ? and model.user.userId = ? and model.memberType = ? and model.currentAddress.localElectionBody.localElectionBodyId is not null", params);
	}
	...
	public List findCadresNotAssignedToBoothInWard(Long wardId, Long userId,
		...
		Object[] params = {wardId, userId,cadreType};
		return getHibernateTemplate().find("select count(model.cadreId) from Cadre model where model.currentAddress.booth.boothId is null" +
				" and model.currentAddress.ward.constituencyId = ? and model.user.userId = ? and model.memberType = ? and model.currentAddress.ward.constituencyId is not null", params);
	}
	...
	public List<Object[]> findActiveCadreForSMS(Long userId,String cadreType,String searchCriteria,String sortOption,String order,Integer startIndex,Integer maxResult)
	{
		...
		queryBuffer.append("from Cadre model where model.user.userId = "+userId+" "+cadreType+" "+searchCriteria+" order by "+sortOption+" " +order);
	...
	public List<Object[]> findNormalCadre(Long userId)
	{
		...
		queryBuffer.append("from Cadre model where model.user.userId = "+userId+" and model.memberType = 'Normal'" );
	...
	public List<Long> findCadreForSMS(Long registrationId,String cadreType,String searchCriteria,String SocailStatus,String genderStr,String mobileStr,String cadreNameStr,String roleStr,String bloodGroupStr, String sortOption,String order,Integer startIndex,Integer maxResult)
	{
		StringBuffer queryBuffer = new StringBuffer("select model.cadreId ");
		queryBuffer.append("from Cadre model where model.registration.registrationId = "+registrationId+" "+cadreType+" "+searchCriteria+" "+SocailStatus+" "+genderStr+" "+mobileStr+" "+cadreNameStr+" "+roleStr+" "+bloodGroupStr+" "+" order by "+sortOption+" " +order);
	...
	@SuppressWarnings("unchecked")
	public List<Long> findTotalCadreCountForSms(Long registrationId,String cadreType,String searchCriteria,String SocailStatus,String genderStr,String mobileStr,String cadreNameStr,String roleStr, String bloodGroupStr)
	{
		StringBuffer queryBuffer = new StringBuffer("select count(model.cadreId) ");
		queryBuffer.append("from Cadre model where model.registration.registrationId = "+registrationId+" "+cadreType+" "+searchCriteria+" "+SocailStatus+" "+genderStr+" "+mobileStr+" "+cadreNameStr+" "+roleStr +" "+bloodGroupStr+" ");
