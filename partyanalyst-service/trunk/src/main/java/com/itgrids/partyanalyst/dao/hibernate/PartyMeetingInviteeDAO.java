...
public List<Object[]> getPublicRepresentativeWiseInvitedCadreCountForMeeting(PartyMeetingsInputVO inputVO){
   ...
   return query.list();
	...
	public List<Long> getPartyMeetingOfInviteeList(Long meetingId,Date fromDate,Date toDate){
		StringBuilder sb = new StringBuilder();
		sb.append("select distinct model.tdpCadre.tdpCadreId from PartyMeetingInvitee model " +
 ...
 public List<Object[]> getPartyMeetingOfAbsentCandidate(Set<Long> absentList,Long locationLevel,Long locationValue,Long meetingType,Long meetingId){
	 StringBuilder sb = new StringBuilder();
	 sb.append("select distinct model.tdpCadre.tdpCadreId,model.tdpCadre.firstname," +
	 		" model.tdpCadre.mobileNo,model.absenteeRemark" +
	 		",model.isAttended,model.partyMeetingInviteeId " +
	 		" ,model.partyMeeting.partyMeetingType.partyMeetingTypeId,model.partyMeeting.partyMeetingType.type" +
	 		//" model.partyMeeting.partyMeetingLevel.partyMeetingLevelId,model.partyMeeting.partyMeetingLevel.level " +
	 		" from PartyMeetingInvitee model " +
	 		" where model.tdpCadre.isDeleted='N' and model.tdpCadre.enrollmentYear=:enrollmentYear " +
	 		" and model.partyMeeting.isActive='Y' ");
	 if(absentList != null && absentList.size()>0){
		 sb.append(" and model.tdpCadre.tdpCadreId in(:absentList) ");
	 }
	 if(locationLevel!=null && locationLevel==1l && locationValue != null){
		 sb.append(" and model.meetingAddress.state.stateId =:locationValue ");
	 }else if(locationLevel!=null && locationLevel==2l && locationValue != null){
		 sb.append(" and model.partyMeeting.meetingAddress.district.districtId =:locationValue "); 
	 }else if(locationLevel!=null && locationLevel==3l && locationValue != null){
		 sb.append(" and model.meetingAddress.constituency.constituencyId =:locationValue ");
	 }else if(locationLevel!=null && locationLevel==4l && locationValue != null){
		 sb.append(" and model.meetingAddress.tehsil.tehsilId =:locationValue ");
	 }else if(locationLevel!=null && locationLevel==5l && locationValue!= null){
		 sb.append(" and  model.meetingAddress.localElectionBody.localElectionBodyId =:locationValue ");
	 }else if(locationLevel!=null && locationLevel==6l && locationValue != null){
		 //sb.append(" and  model.meetingAddress.panchayat.panchayatId =:locationValue ");
	 }else if(locationLevel!=null && locationLevel==7l && locationValue != null){
		 sb.append(" and  model.meetingAddress.panchayat.panchayatId =:locationValue ");
	 }else if(locationLevel!=null && locationLevel==8l && locationValue!= null){
		 sb.append(" and model.meetingAddress.ward.constituencyId =:locationValue ");
	 }else if(locationLevel!=null && locationLevel==9l && locationValue != null){
		// sb.append(" and model.meetingAddress.state.stateId =:locationValue ");
	 }
	 if(meetingType!=null && meetingType>0l){
         sb.append(" and model.partyMeeting.partyMeetingType.partyMeetingTypeId=:meetingType ");
     }	
	 if(meetingId != null && meetingId.longValue()>0l){
		 sb.append(" and model.partyMeeting.partyMeetingId =:meetingId ");
	 }
	 sb.append(" group by model.tdpCadre.tdpCadreId ");
	 Query query = getSession().createQuery(sb.toString());
	 query.setParameter("enrollmentYear", IConstants.CADRE_ENROLLMENT_NUMBER);
	 if(absentList != null && absentList.size()>0){
		 query.setParameterList("absentList", absentList); 
	 }
	 if(locationLevel!=null && locationValue != null  && locationLevel.longValue()>0l){
...
public Long getUpdateMeetingDetails(Long meetingId,Long inviteeId,Long candidateId){
	 StringBuilder sb = new StringBuilder();
	 sb.append(" select model.partyMeetingInviteeId from PartyMeetingInvitee model " +
	 		" where " );
	 if(candidateId != null && candidateId.longValue()>0l){
	  sb.append(" model.tdpCadreId =:candidateId and " );
	 }
	 if(meetingId != null && meetingId.longValue()>0l){
	  sb.append(" model.partyMeeting.partyMeetingId =:meetingId ");
	 }
	 Query query = getSession().createQuery(sb.toString());
	 if(candidateId != null && candidateId.longValue()>0l){
