...
public class PanchayatHamletDAO extends GenericDaoHibernate<PanchayatHamlet,Long> implements IPanchayatHamletDAO{
	...
	{
		return getHibernateTemplate().find("select distinct model.hamlet.hamletName from PanchayatHamlet model where model.panchayat.panchayatId = ? order by model.hamlet.hamletName",panchayatId);
	}
	
	public List<Long> getboothdByPanchayat(Long year,List<Long> panchayatIds)
	{
		Query query = getSession().createQuery("select distinct model1.boothConstituencyElection.booth.boothId from PanchayatHamlet model , " +
				" HamletBoothElection model1 where model.hamlet.hamletId = model1.hamlet.hamletId " +
				" and model1.boothConstituencyElection.booth.year = :year and model.panchayat.panchayatId in (:panchayatIds)");
	...
	@SuppressWarnings("unchecked")
	public List<Object[]> getHamletsListByConstituency(Long constituencyId,Long publicationDateId)
	{
		Query query = getSession().createQuery("select distinct model.hamlet.hamletId,model.hamlet.hamletName,model.panchayat.panchayatId,model2.tehsil.tehsilId from PanchayatHamlet model, Booth model2 where model.panchayat.panchayatId =model2.panchayat.panchayatId and  " +
				" model2.constituency.constituencyId =:constituencyId and model2.publicationDate.publicationDateId =:publicationDateId  )");
	...
	{
		Query query = getSession().createQuery(" select distinct model2.boothConstituencyElection.booth.boothId from PanchayatHamlet model, HamletBoothElection model2 where model.hamlet.hamletId = model2.hamlet.hamletId " +
				" and model.panchayat.panchayatId =:panchayatId and model2.boothConstituencyElection.constituencyElection.election.electionId =:electionId ");
