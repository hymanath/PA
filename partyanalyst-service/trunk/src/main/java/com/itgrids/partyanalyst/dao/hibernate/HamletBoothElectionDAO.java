...
public class HamletBoothElectionDAO extends GenericDaoHibernate<HamletBoothElection, Long> implements IHamletBoothElectionDAO{
	...
	{
		Query query = getSession().createQuery("select model4.panchayat.panchayatId,model.boothConstituencyElection.booth.boothId " +
				"  from HamletBoothElection model,PanchayatHamlet model4 where model4.hamlet.hamletId = model.hamlet.hamletId " +
				" and model.hamlet.hamletId in(select model2.hamlet.hamletId from PanchayatHamlet model2 where model2.panchayat.panchayatId in(:panchayatIds)) " +
	...
	{
		Query query = getSession().createQuery(" select distinct model.boothConstituencyElection.booth.boothId from HamletBoothElection model,PanchayatHamlet model4 where model4.hamlet.hamletId = model.hamlet.hamletId " +
				" and model.hamlet.hamletId in(select model2.hamlet.hamletId from PanchayatHamlet model2 where model2.panchayat.panchayatId in(:panchayatIds)) and " +
				" model.boothConstituencyElection.constituencyElection.election.electionId =:electionId ");
		
		query.setParameterList("panchayatIds", panchayatIdsList);
		query.setParameter("electionId",electionId);
		return query.list();
	}
	...
	public List<Object[]> getMunicipalityNames(List<Long> boothIds)
	{
		Query query = getSession().createQuery("select distinct model.boothId,concat(model.localBody.name,' ',model.localBody.electionType.electionType) from Booth model where " +
				"  model.boothId in (:boothIds)");
		query.setParameterList("boothIds", boothIds);
		return  query.list(); 
	}
	...
	{
		Query query = getSession().createQuery("select distinct PH.panchayat.panchayatId from HamletBoothElection HBE , PanchayatHamlet PH where HBE.hamlet.hamletId = PH.hamlet.hamletId " +
				" and PH.panchayat.tehsil.tehsilId in( :tehsilIds) and HBE.boothConstituencyElection.constituencyElection.election.electionId =:electionId and HBE.boothConstituencyElection.booth.constituency.constituencyId =:constituencyId");
