...
GenericDaoHibernate<PartialBoothPanchayat, Long> implements IPartialBoothPanchayatDAO{
	...
	{
		Query query = getSession().createQuery("Select model from PartialBoothPanchayat model where model.booth.constituency.constituencyId = :constituencyId ");
		query.setParameter("constituencyId",constituencyId);
		return query.list();
	}
	...
	public List<Object[]> getHamletsForPartialHamlets(Long panchayatId,Long publicationId){
		...
		Query query = getSession().createQuery("select distinct pbp.hamlet.hamletId,pbp.hamlet.hamletName from PartialBoothPanchayat pbp"+
				" where pbp.panchayat.panchayatId = :panchayatId and pbp.booth.publicationDate.publicationDateId = :publicationId") ;
		query.setParameter("panchayatId", panchayatId);
		query.setParameter("publicationId", publicationId);
		return query.list();
	}
	...
	{
		Query query = getSession().createQuery("select model.hamlet.hamletId, model.hamlet.hamletName from PartialBoothPanchayat model where " +
				" model.booth.boothId = :boothId");
		query.setParameter("boothId", boothId);
