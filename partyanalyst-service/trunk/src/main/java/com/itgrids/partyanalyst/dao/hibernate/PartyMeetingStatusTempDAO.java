...
public class PartyMeetingStatusTempDAO extends GenericDaoHibernate<PartyMeetingStatusTemp, Long> implements IPartyMeetingStatusTempDAO {
	...
	public PartyMeetingStatusTempDAO() {
	 ...
	 public int deleteAllRecordsFromTemp(){
    	...
    	Query query = getSession().createSQLQuery(" delete from party_meeting_status_temp ");
    	return query.executeUpdate();
    }
	 
	public int setPrimaryKeyAutoIncrementToOneToTemp(){
    	Query query = getSession().createSQLQuery(" ALTER TABLE party_meeting_status_temp AUTO_INCREMENT = 1 ");
    	return query.executeUpdate();
	}
	 ...
	 public int insertPartyofficeAndIvrStatusToTemp(){
	    	...
	    	"  INSERT INTO party_meeting_status_temp(party_meeting_id,party_office_status,ivr_status,third_party_status) " +
	 ...
	 public int updatePartyMeetingStatus1(){
	    	...
	    	Query query = getSession().createSQLQuery("" +
	    	"  UPDATE party_meeting_status_temp SET meeting_status = 'Y' "+
	 ...
	 public int updatePartyMeetingStatus2(){
	    	...
	    	Query query = getSession().createSQLQuery("" +
	    	"  UPDATE party_meeting_status_temp SET meeting_status = 'N' "+ 
			"   WHERE  party_office_status IS NOT NULL AND "+ 
			"	       ivr_status IS NOT NULL AND "+ 
	    ...
	    public int updatePartyMeetingStatus3(){
	    	...
	    	Query query = getSession().createSQLQuery("" +
	    	"  UPDATE party_meeting_status_temp SET meeting_status = 'M' "+
	        "   WHERE  party_office_status IS NOT NULL AND "+
	        "   ivr_status IS NOT NULL AND "+
	        "   party_office_status = 'Y' AND "+
	        "  ivr_status = 'N' ");
	   ...
	   public int updatePartyMeetingStatus9(){
	   	...
	   	Query query = getSession().createSQLQuery("" +
		" UPDATE party_meeting_status_temp SET meeting_status = 'NU' "+ 
		" WHERE  party_office_status IS NULL AND "+
		"	     ivr_status IS NULL ");
	   	return query.executeUpdate();
	   }
	  ...
	  public int setInsertedDate(Date currentDateTime){
		   Query query = getSession().createSQLQuery(" UPDATE party_meeting_status_temp SET inserted_time =:currentDateTime  ");
	  ...
	  public int insertDataToPartyMeetingStatusFromTemp(){
		  ...
		  " select party_meeting_id,party_office_status,ivr_status,third_party_status,meeting_status,inserted_time from party_meeting_status_temp ");
		  return query.executeUpdate();
	  }
}
