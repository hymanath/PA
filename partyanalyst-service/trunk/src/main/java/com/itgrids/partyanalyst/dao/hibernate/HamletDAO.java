...
public class HamletDAO extends GenericDaoHibernate<Hamlet, Long> implements IHamletDAO {
	...
	{
		Query query = getSession().createQuery("select distinct cast(model.township.tehsil.tehsilId,string) " +
				" from Hamlet model where model.hamletId in (:hamletIds)");
	...
	{
		Query query = getSession().createQuery("select model.hamletId, model.hamletName,model.township.tehsil.tehsilId from Hamlet model where model.township.tehsil.district.state.stateId = :stateId ");
