...
public class VoterModificationDAO extends GenericDaoHibernate<VoterModification,Long> implements IVoterModificationDAO{
	...
	public List<Object[]> getAllSelectedVotersDetails(Long constituencyId, List<Long> publicationIdsList, Long locationValue, String type, String queryStr){
	     ...
	     */
			...
			str.append(" select distinct model.voter.voterId,model.voter.name,model.voter.gender, model.voter.age, model.voter.relativeName, model.voter.relationshipType, ");
			str.append(" model2.booth.boothId, model2.booth.partNo, model2.booth.villagesCovered, ");
	...
	{
		Query query = getSession().createQuery("select model.voterModificationId,model.voter.voterId, model.status, model.partNo, model.constituency.constituencyId,model.publicationDate.publicationDateId,model.voterStatus.voterStatusId from VoterModification model where " +
