...
public class StaticDataService implements IStaticDataService {
			...
			*/
			...
			for(int k=0;k<constituencyWinnerInfoVOList.size();k++){	
				for(int l=0;l<parliamentCandidates.size();l++){	
					...
					if(constituencyWinnerInfoVOList.get(k).getParliamentConstituencyId().equals(new Long(parliamentCandidatesObj[3].toString()))){
	...
	public List<TeshilPartyInfoVO> getMandalWisePartyReport(String electionType,String electionYear,Long districtId){
		...
		try{
			if(electionYear.equalsIgnoreCase("null")){
				Long stateId = districtDAO.get(districtId).getState().getStateId();
				electionYear = electionDAO.findLatestElectionAssemblyElectionYearForState(IConstants.ASSEMBLY_ELECTION_TYPE,stateId,IConstants.ELECTION_SUBTYPE_MAIN)
						.get(0).toString() ;
			}
			if(log.isDebugEnabled())
	...
	public ConstituencyResultsInElectionVO getAllCandidatesResultsInConstituency(final Long constituencyId,final String electionYear,final String electionType){
		...
		try{
			...
			if(electionType.equals(IConstants.PARLIAMENT_ELECTION_TYPE)){
				...
				java.util.ListIterator li = result.listIterator();
				 while(li.hasNext()){
					 ...
					 //set to main VO
					 constituencyElectionCompleteResults.setConstituencyId(Long.parseLong(parms[0].toString()));
					 constituencyElectionCompleteResults.setConstituencyName(parms[1].toString());
					 constituencyElectionCompleteResults.setElectionInfo(electionInfo);
					 constituencyElectionCompleteResults.setStateId(Long.parseLong(parms[2].toString()));
					 constituencyElectionCompleteResults.setStateName(parms[3].toString());
				 }
				...
				if(electionType.equals(IConstants.CORPORATION_ELECTION_TYPE) || electionType.equals(IConstants.MUNCIPLE_ELECTION_TYPE) || electionType.equals(IConstants.GREATER_ELECTION_TYPE)){
				 ...
				 while(li.hasNext()){
					 ...
					 constituencyElectionCompleteResults.setConstituencyId(Long.parseLong(parms[0].toString()));
					 constituencyElectionCompleteResults.setConstituencyName(parms[1].toString());
					 constituencyElectionCompleteResults.setDistrictId(Long.parseLong(parms[2].toString()));
					 constituencyElectionCompleteResults.setDistrictName(parms[3].toString());
					 constituencyElectionCompleteResults.setStateId(Long.parseLong(parms[4].toString()));
					 constituencyElectionCompleteResults.setStateName(parms[5].toString());
					 constituencyElectionCompleteResults.setElectionInfo(electionInfo);
			 ...
			 while(resultIterator.hasNext()){
				 ...
				 if(Long.parseLong(parms[4].toString())==1l){
					 ...
					 CandidateElectionCompleteResultsVO wonCandidateResults = new CandidateElectionCompleteResultsVO();
						 wonCandidateResults.setCandidateId(Long.parseLong(parms[0].toString()));
						 ...
						 wonCandidateResults.setVotesPercentAsString(parms[3].toString());
						 wonCandidateResults.setRank(Long.parseLong(parms[4].toString()));
						 //set party details
						 PartyElectionDetailsVO partyElectionDetails = new PartyElectionDetailsVO();
						 if(!(parms[5]==null)){
							 partyElectionDetails.setPartyId(Long.parseLong(parms[5].toString()));
						 }
						 if(!(parms[6]==null)){
						 ...
						 wonCandidateResults.setPartyElectionDetails(partyElectionDetails);
						 					 
						 //set won candidate results to VO
						 partyElectionCompleteResults.setCandidateElectionCompleteResults(wonCandidateResults);
					 ...
					 CandidateElectionCompleteResultsVO oppCandidateResults = new CandidateElectionCompleteResultsVO();
				 	 oppCandidateResults.setCandidateId(Long.parseLong(parms[0].toString()));
					 ...
					 String candidateName = parms[1].toString();
						if(candidateName.contains("\n")){
							...
							oppCandidateResults.setCandidateLastName(parms[1].toString());
						}
					 Double votesEarned = (Double)parms[2];
					 Long votesEarn = votesEarned.longValue();
					 totalVotesGainedByAllCandidates+=votesEarn;
					 oppCandidateResults.setCandidateVotesEarned(votesEarn);
					 oppCandidateResults.setVotesPercentAsString(parms[3].toString());
					 oppCandidateResults.setRank(Long.parseLong(parms[4].toString()));
					 PartyElectionDetailsVO partyElectionDetails = new PartyElectionDetailsVO();
					 if(!(parms[5]==null)){
						 partyElectionDetails.setPartyId(Long.parseLong(parms[5].toString()));
					 }
					 ...
					 partyElectionDetails.setPartyLongName(parms[7].toString());
					 partyElectionDetails.setPartyShortName(parms[8].toString());
					 oppCandidateResults.setPartyElectionDetails(partyElectionDetails);
					 candidateElectionCompleteResultsList.add(oppCandidateResults);
				 }
			 }
			 if(totalVoters.equals(0d)){
				 constituencyElectionCompleteResults.setTotalVoters(new Long(0));
				 constituencyElectionCompleteResults.setTotalVotesPolled(totalVotesGainedByAllCandidates);
				 constituencyElectionCompleteResults.setTotalVotersPercentInString("N/A");
			 }else{
				 constituencyElectionCompleteResults.setTotalVoters(totalVoters.longValue());	
				 if(totalPolledVotes.longValue()!=0l){
					 constituencyElectionCompleteResults.setTotalVotesPolled(totalPolledVotes.longValue());
					 constituencyElectionCompleteResults.setTotalVotersPercentInString(new BigDecimal((totalPolledVotes/totalVoters)*100).setScale(2, BigDecimal.ROUND_HALF_UP).toString());
				 }else{
					 constituencyElectionCompleteResults.setTotalVotesPolled(totalVotesGainedByAllCandidates);
					 constituencyElectionCompleteResults.setTotalVotersPercentInString(new BigDecimal((totalVotesGainedByAllCandidates/totalVoters)*100).setScale(2, BigDecimal.ROUND_HALF_UP).toString());
				 } 
			 }	 
			  
			 
			 //set opposition
			 partyElectionCompleteResults.setCandidateElectionCompleteResultsList(candidateElectionCompleteResultsList);
			 constituencyElectionCompleteResults.setPartyElectionCompleteResults(partyElectionCompleteResults);
			 
			 //for margin votes
			 if(constituencyElectionCompleteResults != null && constituencyElectionCompleteResults.getPartyElectionCompleteResults() != null){
				 for(CandidateElectionCompleteResultsVO completeRes:constituencyElectionCompleteResults.getPartyElectionCompleteResults().getCandidateElectionCompleteResultsList()){
					 if(completeRes.getRank().equals(new Long(2))){
						 Long votesMarginVal = constituencyElectionCompleteResults.getPartyElectionCompleteResults().getCandidateElectionCompleteResults().getCandidateVotesEarned() - completeRes.getCandidateVotesEarned();
						//Original written Double votesMarginPercent = new Double(constituencyElectionCompleteResults.getPartyElectionCompleteResults().getCandidateElectionCompleteResults().getVotesPercentAsString()) - new Double(completeRes.getVotesPercentAsString());
						 
						// System.out.println(" ........................ Votes Margin Gained :" + votesMarginVal);
						// System.out.println(" ........................ Votes Margin % Gained :" + votesMarginPercent);						 
						// System.out.print(constituencyElectionCompleteResults.getPartyElectionCompleteResults().getCandidateElectionCompleteResults().getCandidateVotesEarned()+" - \t"+completeRes.getCandidateVotesEarned()+" = \t"+votesMarginVal);
						 Double votesMarginPercent = (votesMarginVal.doubleValue()/constituencyElectionCompleteResults.getTotalVotesPolled()*100);
						// System.out.println("\t"+votesMarginPercent);
						 constituencyElectionCompleteResults.getPartyElectionCompleteResults().getCandidateElectionCompleteResults().setMarginVotesEarned(votesMarginVal);
						 constituencyElectionCompleteResults.getPartyElectionCompleteResults().getCandidateElectionCompleteResults().setMarginVotesPercentAsString(new BigDecimal(votesMarginPercent).setScale(2, BigDecimal.ROUND_HALF_UP).toString());
					 }
					 Long oppMarginVotesVal = completeRes.getCandidateVotesEarned() - constituencyElectionCompleteResults.getPartyElectionCompleteResults().getCandidateElectionCompleteResults().getCandidateVotesEarned();
					 Double oppMarginPercent = new Double(completeRes.getVotesPercentAsString()) - new Double(constituencyElectionCompleteResults.getPartyElectionCompleteResults().getCandidateElectionCompleteResults().getVotesPercentAsString());
					 completeRes.setMarginVotesEarned(oppMarginVotesVal);
					 completeRes.setMarginVotesPercentAsString(new BigDecimal(oppMarginPercent).setScale(2, BigDecimal.ROUND_HALF_UP).toString());
				 }
			...
			constituencyResultsInElection.setConstituencyElectionCompleteResults(constituencyElectionCompleteResults);
	...
	public MandalAllElectionDetailsVO getAllZptcOrMptcWinnerForADistrictForLatestYear(Long districtId,String electionYear,String electionType) {
		...
		try{
			...
			List successorCandidate = nominationDAO.findAllZPTCsOrMPTCsInaDistrict(districtId,electionType,successorRank,electionYear);
			
			log.info("Making nominationDAO.findAllZPTCsInaDistrict DAO call");
			List winningCandidate = nominationDAO.findAllZPTCsOrMPTCsInaDistrict(districtId,electionType,winnerRank,electionYear);
			
			log.info("Calling populateElectionsData() method..");
			allVotersDetails = populateElectionsData(winningCandidate,successorCandidate,0,0,electionType);			
	...
	public MandalAllElectionDetailsVO getAllZptcsOrMptcsCandidatesForADistrictForSelectedYear(Long districtId,String electionYear,int lostFlag,String electionType) {
		...
		try{	
			...
			List successorCandidate = nominationDAO.findAllZPTCsOrMPTCsInaDistrict(districtId,IConstants.ZPTC_ELECTION_TYPE,successorRank,electionYear);
			
			log.info("Making nominationDAO.findAllZPTCsInaDistrict DAO call");
			List winningCandidate = nominationDAO.findAllZPTCsOrMPTCsInaDistrict(districtId,IConstants.ZPTC_ELECTION_TYPE,winnerRank,electionYear);
			
			log.info("Making nominationDAO.findAllZptcCandidatesInaDistrict DAO call");
			List allCandidates = nominationDAO.findAllZptcOrMptcCandidatesInaDistrict(districtId,IConstants.ZPTC_ELECTION_TYPE,electionYear);
	...
	public CandidateDetailsVO getCandidatesPartyInfoForAnElectionType(String electionType,String electionYear,String resultsCategory,String electionLevel,Long locationId,Long partyId,Long stateId){
		...
		try{
				...
				if(locationId!=0){
					...
					if(resultsCategory.equalsIgnoreCase(IConstants.ALL_CANDIDATES) || resultsCategory.equalsIgnoreCase(IConstants.LOST_CANDIDATES)){
						if(partyId!=0){
							...
							mandalAllElectionDetailsVo = getAllZptcsOrMptcsCandidatesForADistrictForSelectedYear(locationId,electionYear,lostFlag,IConstants.ZPTC_ELECTION_TYPE);
						}						
						...
						if(partyId!=0){
							...
							mandalAllElectionDetailsVo = getAllZptcOrMptcWinnerForADistrictForLatestYear(locationId,electionYear,IConstants.ZPTC_ELECTION_TYPE);
						}	
					}
				...
				if(locationId!=0){
					...
					if(resultsCategory.equalsIgnoreCase(IConstants.ALL_CANDIDATES) || resultsCategory.equalsIgnoreCase(IConstants.LOST_CANDIDATES)){		
						if(partyId!=0){
							...
							mandalAllElectionDetailsVo = getAllZptcsOrMptcsCandidatesForADistrictForSelectedYear(locationId,electionYear,lostFlag,IConstants.MPTC_ELECTION_TYPE);
						}						
					}else if(resultsCategory.equalsIgnoreCase(IConstants.WINNER_CANDIDATES)){
						if(partyId!=0){
							flag = 1;
							...
							mandalAllElectionDetailsVo = getAllZptcOrMptcWinnerForADistrictForLatestYear(locationId,electionYear,IConstants.MPTC_ELECTION_TYPE);
						}	
					}
					candidateDetailsVO.setMandalAllElectionDetailsVO(mandalAllElectionDetailsVo);
			...
			//Local Election Body
				...
				if(partyId==0l){
					candidateDetailsVO = getCandidatesInfoForAnElectionType(electionType,electionYear,resultsCategory,electionLevel,locationId,stateId);						
				}else{
					//candidateDetailsVO = getCandidatesWinnerInfoForAnElectionTypes(IConstants.ASSEMBLY_ELECTION_TYPE,electionYear,resultsCategory,electionLevel,locationId,partyId,stateId);
				}		
			}
				...
				if(resultsCategory.equalsIgnoreCase(IConstants.WINNER_CANDIDATES)){
					if(electionLevel.equalsIgnoreCase(IConstants.COUNTRY_LEVEL)){
						...
						//for Local election Bodys
							 ...
							 winnerCandidateResult = nominationDAO.findAllCandidatesForAnElectionBytheElectionYearByRankForLocalBody(electionYear,stateId,electionType,winnerCandidateRank);
							 successorCandidateResult = nominationDAO.findAllCandidatesForAnElectionBytheElectionYearByRankForLocalBody(electionYear,stateId,electionType,successorCandidateRank);
							...
							//for local election bodys
								...
								listOfConstituencies = getConstituenciesForDistrictByType(locationId,stateId,electionYear,electionType);
								if(listOfConstituencies != null && listOfConstituencies.length() > 0){
									winnerCandidateResult = nominationDAO.getCandidatesInfoForTheGivenConstituencyBasedOnRankForLocalBodys(listOfConstituencies.substring(1),electionYear,electionType,winnerCandidateRank);
									successorCandidateResult = nominationDAO.getCandidatesInfoForTheGivenConstituencyBasedOnRankForLocalBodys(listOfConstituencies.substring(1),electionYear,electionType,successorCandidateRank);
								}
							}
					...
					if(electionLevel.equalsIgnoreCase(IConstants.COUNTRY_LEVEL)){
						...
						//Local Body ELections
							...
							if(listOfConstituencies != null && listOfConstituencies.length() > 0){
								winnerCandidateResult = nominationDAO.getCandidatesInfoForTheGivenConstituencyBasedOnRankForLocalBodys(listOfConstituencies.substring(1),electionYear,electionType,winnerCandidateRank);
								successorCandidateResult = nominationDAO.getCandidatesInfoForTheGivenConstituencyBasedOnRankForLocalBodys(listOfConstituencies.substring(1),electionYear,electionType,successorCandidateRank);
								allCandidateResult = nominationDAO.getCandidatesInfoForTheGivenConstituencyForLocalBody(listOfConstituencies.substring(1),electionYear,electionType);
							}
						}	
					}else if(electionLevel.equalsIgnoreCase(IConstants.CONSTITUENCY_LEVEL)){
						winnerCandidateResult = nominationDAO.getCandidatesInfoForTheGivenConstituencyBasedOnRank(locationId.toString(),electionYear,electionType,winnerCandidateRank);
		...
		try{
			List list = null;
			 ...
			 if(electionType.equalsIgnoreCase(IConstants.ASSEMBLY_ELECTION_TYPE) || electionType.equalsIgnoreCase(IConstants.PARLIAMENT_ELECTION_TYPE) ||
					 electionType.equalsIgnoreCase(IConstants.ZPTC_ELECTION_TYPE) || electionType.equalsIgnoreCase(IConstants.MPTC_ELECTION_TYPE) ){
				 list = nominationDAO.getAllPartiesForAnElectionYear(electionYear,electionType);
			 }else{
				 list = nominationDAO.getAllPartiesForAnElectionYearForLocalBody(electionYear,electionType);
			 }
	...
	public DistrictWisePartyResultVO getElectionResultsForDistrict(Long electionScopeId, Long districtId){
		...
		DistrictWisePartyResultVO districtWisePartyResultVO = getDistrictWiseElectionReport(electionScopeId,districtId);
		List<PartyResultVO> parList  = districtWisePartyResultVO.getPartyElectionResultsList();
		Set<String> party = new HashSet<String>(0);
		List list = new ArrayList(0);
		List<SelectOptionVO> allElecList = new ArrayList<SelectOptionVO>(0);
		if(districtWisePartyResultVO.getElectionYearsAndType()!=null && districtWisePartyResultVO.getElectionYearsAndType().size()!=0){
			for(SelectOptionVO selLIst : districtWisePartyResultVO.getElectionYearsAndType()){
		...
		if(electionScopeId == 0){
			list = nominationDAO.getAllElectionIdsAndYearsForADistrict(districtId);		
			allElecList.addAll(getElectionTypes(list));	
		}else if(electionScopeId != 1){
			list = partyElectionDistrictResultDAO.getAllElectionResultsInDistrict(electionScopeId,districtId);
		...
		for(int i=0; i<allElecList.size(); i++){
			...
			for(PartyResultVO result : parList){
				List<ElectionResultVO> elecList = result.getElectionWiseResults();
				for(ElectionResultVO resultVO : elecList){
					if(electYear.equalsIgnoreCase(resultVO.getElectionYear()) && electType.equalsIgnoreCase(resultVO.getElectionType())){	
	...
	/**
	 * This method can be used to get a report of different elections that 
	 * are happened in a particular district.
	 * 
	 * @return DistrictWisePartyResultVO
	...
	public TeshilPartyInfoVO getAllPartyTrendsForAllMuncipalitiesInADistrict(String electionType,Long districtId){
		 ...
		 try{
			 ...
			 allMuncipalities = getLocalElectionPartyDetails(result,latestMuncipalElectionYear,electionType);					
	...
	/**
	 * This method retrives all the MandalIds based on election year for a constituency.
	 * 
	 * @param constituencyId
	 * @param electionYear
	 * @return 
	 */
	public StringBuilder getMandalsForAConstituencyBasedOnElectionYear(Long constituencyId,Long electionYear){
		// This below DAO call retrieves all the latest mandal's for a constituency.
		try{
			List list = null;
			StringBuilder tehsilIds = new StringBuilder();	
			list = delimitationConstituencyMandalDAO.getMandalDetailsForAConstituency(constituencyId,electionYear);
			for(int i=0;i<list.size();i++){
				Object[] parms = (Object[])list.get(i);
			...
			log.debug("Exception raised while making DelimitationConstituencyMandalDAO call");
			return null;
		}		
	}
	...
	public List<ElectionResultPartyVO> getAllMandalElectionInformationForAConstituency(Long constituencyId,int flag){
		...
		//flag = 1 specifies that to get Data only for the main parties like TRS,TDP,INC,BJP,PRP i.e., ByElections Parties that are defined in IConstants.BYE_ELECTIONS_STATIC_PARTIES
		//flag = 0 specifies that to get Data for all the parties participated in that constituency

		StringBuilder tehsilIds = new StringBuilder();		
		List<ElectionResultPartyVO> electionResults = new ArrayList<ElectionResultPartyVO>(0);	
		List list = null;
		...
		try{			
			...
			electionResult = getPartyDetailsForTheGivenElectionYearInAConstituency(0l,tehsilIds,IConstants.PREVIOUS_ELECTION_YEAR,IConstants.ASSEMBLY_ELECTION_TYPE,flag); 
			if(electionResult.getResultStatus().getResultCode()!=1){
				electionResults.add(electionResult);
			}
			
			electionResult = getPartyDetailsForTheGivenElectionYearInAConstituency(0l,tehsilIds,IConstants.PREVIOUS_ELECTION_YEAR,IConstants.PARLIAMENT_ELECTION_TYPE,flag);
			if(electionResult.getResultStatus().getResultCode()!=1){
				electionResults.add(electionResult);
			}
			...
			electionResult =  getPartyDetailsForTheGivenElectionYearInAConstituency(0l,tehsilIds,IConstants.PRESENT_ELECTION_YEAR,IConstants.PARLIAMENT_ELECTION_TYPE,flag);
			if(electionResult.getResultStatus().getResultCode()!=1){
				electionResults.add(electionResult);
			}
			}
			
			electionResult =  getPartyDetailsForTheGivenElectionYearInAConstituency(0l,tehsilIds,IConstants.BYE_ELECTION_YEAR_2008_PARLIAMENT,IConstants.PARLIAMENT_ELECTION_TYPE,flag);
			if(electionResult.getResultStatus().getResultCode()!=1){
				electionResults.add(electionResult);
			}
			electionResult =  getPartyDetailsForTheGivenElectionYearInAConstituency(0l,tehsilIds,IConstants.BYE_ELECTION_YEAR_2006_PARLIAMENT,IConstants.PARLIAMENT_ELECTION_TYPE,flag);
			if(electionResult.getResultStatus().getResultCode()!=1){
				electionResults.add(electionResult);
			}
	...
	/**
	 * This method retrives Votes Percentage Obtained for different Bye-Election parties in the constituency based on latest mandals for Zptc's and Mptc's Elections.
	 * @author Ravi Kiran.Y
	 * @serialData 08-07-10
	 * @param tehsilIds
	 * @param electionType
	 ...
	 * @return
	 */
	public ElectionResultPartyVO getMptcOrZptcElectionVotesPercentageForDifferentParties(StringBuilder tehsilIds,String electionType,String electionYear,int flag,Long constituencyId){
		//flag = 1 specifies that to get Data only for the main parties like TRS,TDP,INC,BJP,PRP i.e., ByElections Parties that are defined in IConstants.BYE_ELECTIONS_STATIC_PARTIES
		//flag = 0 specifies that to get Data for all the parties participated in that constituency
		List<TeshilPartyInfoVO> tehsilVO = new ArrayList<TeshilPartyInfoVO>(0);
		ElectionResultPartyVO electionResult = new ElectionResultPartyVO();
		tehsilVO =  constituencyPageService.getTehsilPartyInfoForAConstituency(tehsilIds,electionYear,electionType,constituencyId);
		if(tehsilVO!=null){
			electionResult = populateDataForLocalElections(tehsilVO,electionYear,electionType,flag);
		}
	...
	public ElectionResultPartyVO getPartyDetailsForTheGivenElectionYearInAConstituency(Long constituencyId,StringBuilder tehsilIds,String electionYear,String electionType,int flag){
		BigDecimal percentage = new BigDecimal(0.0);
		Long totalVotes=0l;
		ElectionResultPartyVO candidateElectionResult = new ElectionResultPartyVO();
		List<CandidateElectionResultVO> candidateElectionResultsVO = null;
		...
		try{
			...
			if(constituencyId != null && constituencyId > 0l){
				list2=nominationDAO.getResultsForElectionInConstituency(constituencyId,electionYear);
			...
			for(int j=0;j<partiesList.length;j++){
				...
				for(int i=0;i<partyResult.size();i++){					
					...
					if(IConstants.ASSEMBLY_ELECTION_TYPE.equalsIgnoreCase(partyResult.get(i).getElectionType()) 
							|| IConstants.PARLIAMENT_ELECTION_TYPE.equalsIgnoreCase(partyResult.get(i).getElectionType())
							&& !"2008".equalsIgnoreCase(partyResult.get(i).getElectionYear()) 
							&& !"2006".equalsIgnoreCase(partyResult.get(i).getElectionYear())){
						List<CandidateElectionResultVO> candidateResults = getProcessedAlliancePartiesResults(partyResult.get(i).getCandidateElectionResultsVO(),partyResult.get(i).getElectionType(),partyResult.get(i).getElectionYear());
					...
					if(partyResult.get(i) != null && partyResult.get(i).getCandidateElectionResultsVO() != null){
				...
				List<SelectOptionVO> partiesListVo = new ArrayList<SelectOptionVO>(0);
	...
	/**
	 * This method retrives all the muncipal/corporation elections that are happened in the constituency.  
	 * @author Ravi Kiran.Y
	 * @param constituencyId
	 * @param electionType
	 * @return
	...
	public TeshilPartyInfoVO getLocalElectionDetailsForAConstituency(Long constituencyId,String electionType){
		...
		try{
				...
				//The below line calls getAllLatestMandalsForAConstituency() method to get all the tehsils for a constituency.
				StringBuilder tehsilIds = getAllLatestMandalsForAConstituency(constituencyId);
				
				//The below line makes a dao call to get the latest election year for the corresponding election type(muncipality/corporation)
				List latestYearResult = electionDAO.findLatestElectionYear(electionType);		
				latestMuncipalElectionYear = latestYearResult.get(0).toString();		
				
				//The below line makes a dao call to get the parties votes for the corresponding election type(muncipality/corporation)
	...
	public List<PartyResultVO> getPartyVotesPercentageInAConstituency(Long constituencyId,String all,String[] choices){
		...
		if(all.equalsIgnoreCase("all")){			
			...
			for(int i=0;i<choices.length;i++){				
				...
				if(choices[i].equalsIgnoreCase("PC")){
					ElectionResultPartyVO electionResult = new ElectionResultPartyVO();
					partyResult.add(getAssemblyOrParliamentElectionVotesPercentageForDifferentParties(0l,tehsilIds,IConstants.PARLIAMENT_ELECTION_TYPE,IConstants.PREVIOUS_ELECTION_YEAR,flag));
					partyResult.add(getAssemblyOrParliamentElectionVotesPercentageForDifferentParties(0l,tehsilIds,IConstants.PARLIAMENT_ELECTION_TYPE,IConstants.PRESENT_ELECTION_YEAR,flag));
					electionResult =  getPartyDetailsForTheGivenElectionYearInAConstituency(0l,tehsilIds,IConstants.BYE_ELECTION_YEAR_2008_PARLIAMENT,IConstants.PARLIAMENT_ELECTION_TYPE,flag);
					if(electionResult.getResultStatus().getResultCode()!=1){
						partyResult.add(electionResult);
					}
				...
				if(choices[i].equalsIgnoreCase(IConstants.ZPTC_ELECTION_TYPE)){
					partyResult.add(getMptcOrZptcElectionVotesPercentageForDifferentParties(tehsilIds,IConstants.ZPTC_ELECTION_TYPE,IConstants.LOCAL_ELECTIONS_PREVIOUS_ELECTION_YEAR,constituencyId,flag));
					partyResult.add(getMptcOrZptcElectionVotesPercentageForDifferentParties(tehsilIds,IConstants.ZPTC_ELECTION_TYPE,IConstants.LOCAL_ELECTIONS_PRESENT_ELECTION_YEAR,constituencyId,flag));
				}else
				if(choices[i].equalsIgnoreCase(IConstants.MPTC_ELECTION_TYPE)){
					partyResult.add(getMptcOrZptcElectionVotesPercentageForDifferentParties(tehsilIds,IConstants.MPTC_ELECTION_TYPE,IConstants.LOCAL_ELECTIONS_PREVIOUS_ELECTION_YEAR,constituencyId,flag));
					partyResult.add(getMptcOrZptcElectionVotesPercentageForDifferentParties(tehsilIds,IConstants.MPTC_ELECTION_TYPE,IConstants.LOCAL_ELECTIONS_PRESENT_ELECTION_YEAR,constituencyId,flag));
				}				
			}	
		...
		if(party != null && party.size() > 0 && party.get(0) != null){
			...
			if(rflag == true){
				...
				List<ElectionResultVO> electionWiseRes = resVO.getElectionWiseResults();
	...
	/**
	 * This method retrives Votes Percentage Obtained for different Bye-Election parties in the constituency based on latest mandals for Mptc and Zptc Elections.
	 * @param tehsilIds
	 * @param electionType
	 * @param electionYear
	 * @param constituencyId
	 ...
	 * @return
	 */
	public ElectionResultPartyVO getMptcOrZptcElectionVotesPercentageForDifferentParties(StringBuilder tehsilIds,String electionType,String electionYear,Long constituencyId,int flag){
		//flag = 1 specifies that to get Data only for the main parties like TRS,TDP,INC,BJP,PRP i.e., ByElections Parties that are defined in IConstants.BYE_ELECTIONS_STATIC_PARTIES
		//flag = 0 specifies that to get Data for all the parties participated in that constituency
		ElectionResultPartyVO electionResult = new ElectionResultPartyVO();
		List<TeshilPartyInfoVO> tehsilVO = constituencyPageService.getTehsilPartyInfoForAConstituency(tehsilIds,electionYear,electionType,constituencyId);
		if(tehsilVO!=null){
			electionResult = populateDataForLocalElections(tehsilVO,electionYear,electionType,flag);
		}
		return electionResult;
	...
	/**
	 * This method retrives Votes Percentage Obtained for different Bye-Election parties in the constituency based on latest mandals for Assembly and Parliament Elections.
	 * @author Ravi Kiran.Y
	 * @serialData 08-07-10
	 * @param tehsilIds
	 * @param electionType
	 ...
	 * @return
	 */
	public ElectionResultPartyVO getAssemblyOrParliamentElectionVotesPercentageForDifferentParties(Long constituencyId,StringBuilder tehsilIds,String electionType,String electionYear,int flag){		
		//flag = 1 specifies that to get Data only for the main parties like TRS,TDP,INC,BJP,PRP i.e., ByElections Parties that are defined in IConstants.BYE_ELECTIONS_STATIC_PARTIES
		//flag = 0 specifies that to get Data for all the parties participated in that constituency
		ElectionResultPartyVO electionResult = new ElectionResultPartyVO();
		electionResult =  getPartyDetailsForTheGivenElectionYearInAConstituency(constituencyId,tehsilIds,electionYear,electionType,flag);		
		return electionResult;
		//electionResult =  getAssemblyOrParliamentElectionVotesPercentageForDifferentParties(tehsilIds,IConstants.PARLIAMENT_ELECTION_TYPE,IConstants.PRESENT_ELECTION_YEAR,flag);
	...
	public List<CandidateElectionResultVO> getProcessedAlliancePartiesResults(List<CandidateElectionResultVO> elecResultVO,String elecType,String elecYear){
		...
		if(elecResultVO != null &&  elecResultVO.size() > 0){
			...
			else if(elecType.equalsIgnoreCase(IConstants.PARLIAMENT_ELECTION_TYPE))
			...
			if(partyResults != null && !partyResults.isEmpty()){
				...
				for(Long party:partyIds){
				 ...
				 List<SelectOptionVO> alliancParties =  getAlliancePartiesAsVO(elecYear, elecTypeId, party);
				 ...
				 if(alliancParties != null && alliancParties.size() > 0){
					 ...
					 for(SelectOptionVO alliancPar:alliancParties){
						 ...
						 if(partyResults.containsKey(alliancPar.getId()) 
								 && !addedParties.containsKey(alliancPar.getId())){
							 
							 allianc.add(partyResults.get(alliancPar.getId()));
							
							 addedParties.put(alliancPar.getId(), "Party Added");
						 }
					 ...
					 if(partyResults.containsKey(party)){
					  candidateElectionResultsVO.add(partyResults.get(party)); 
	...
	public CandidateElectionResultVO getGroupedResultForAllianc(List<CandidateElectionResultVO> alliancRes){
		...
		if(alliancRes != null && alliancRes.size() > 0){
			...
			if(alliancRes.size() == 1)
			   ...
			   Collections.sort(alliancRes,new CandidateElecResultVOComparator());
			
			   Double totPercentage = new Double(0);
			   for(int i=0;i<alliancRes.size();i++){
				   
				   totPercentage+=new Double(alliancRes.get(i).getVotesPercentage());
			   }
			   
			   if(totPercentage > new Double(0)){
				   totPercentage = new BigDecimal(totPercentage).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
				   
				   alliancRes.get(0).setVotesPercentage(totPercentage.toString());
	...
	public Map<Long,CandidateElectionResultVO> getPartyResultsInMap(List<CandidateElectionResultVO> candResults){
