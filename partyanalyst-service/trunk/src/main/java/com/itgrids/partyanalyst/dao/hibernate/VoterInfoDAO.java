...
public class VoterInfoDAO extends GenericDaoHibernate<VoterInfo, Long> implements IVoterInfoDAO{
	...
	public List<Object[]> getVoterCadreDetailsBySearchCriteria(Long stateId, String locationType,List<Long> locationIdsList)
	{
		...
		else if(locationType != null && locationType.equalsIgnoreCase(IConstants.PANCHAYAT))
		{
			...
			str.append(" group by model2.panchayatId order by model2.panchayatName ");
		}
		else if(locationType != null && locationType.equalsIgnoreCase(IConstants.LOCAL_ELECTION_BODY))
		{
			...
			str.append(" group by model2.localElectionBodyId order by model2.name ");
		}
		else if(locationType != null && locationType.equalsIgnoreCase(IConstants.WARD))
		{
			...
			str.append(" and model.reportLevelValue in (:locationIdsList)  ");			
			str.append(" group by model2.constituencyId order by model2.name ");
		}
		else if(stateId != null && stateId.longValue() == 0L) //AP & TS
		{
			isStateWise = true;
			str.append(" select distinct D.districtId,model.totalVoters  ");
			str.append(" ,D.districtName from VoterInfo model,Constituency model2,District D where model2.district.districtId = D.districtId and model2.constituencyId = model.reportLevelValue and (D.districtId between 1 and 23) ");
			str.append(" group by D.districtId order by D.districtName ");
		}
		else if(stateId != null && stateId.longValue() == 1L) //AP
		{
			isStateWise = true;
			str.append(" select distinct D.districtId,model.totalVoters  ");
			str.append(" ,D.districtName from VoterInfo model,Constituency model2,District D where model2.district.districtId = D.districtId and model2.constituencyId = model.reportLevelValue and (D.districtId between 11 and 23) ");
			str.append(" group by D.districtId order by D.districtName ");
		}
		else if(stateId != null && stateId.longValue() == 2L) //TS
		{
			isStateWise = true;
			str.append(" select distinct D.districtId,model.totalVoters  ");
			str.append(" ,D.districtName from VoterInfo model,Constituency model2,District D where model2.district.districtId = D.districtId and model2.constituencyId = model.reportLevelValue and (D.districtId between 1 and 10) ");
			str.append(" group by D.districtId order by D.districtName ");
		}		
