   ...
   /* public List<SelectOptionVO> getParliamentConstisByElectionYear(String year,List<Long> parliamentIds){
	...
	public List<SelectOptionVO> getConstituenciesForElectionYearAndTypeWithUserAccessByStateId(Long electionScope,Long stateId,Long userId,Long electionYear)
	{
		try{
			...
			List<Object[]> conList = (List<Object[]>)userConstituencyAccessInfoDAO.findByElectionScopeUserState(electionScope,userId,stateId);
			...
			if(conList != null && conList.size() > 0)
			{
				if(electionScope.intValue() == 1)
				{
					...
					{
						Long constuencyId = (Long)consList[0];
						List<Object[]>constList = delimitationConstituencyDAO.getLatestConstituenciesByElectionTypeAndYear(electionScope,constuencyId,electionYear);
