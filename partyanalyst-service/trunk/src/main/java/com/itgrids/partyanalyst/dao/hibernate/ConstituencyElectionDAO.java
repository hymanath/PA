...
public class ConstituencyElectionDAO extends GenericDaoHibernate<ConstituencyElection, Long> implements
	...
	public List<Object[]> getAllElectionIdsForConstituency(Long constituencyId){
		...
		Query query = getSession()
				.createQuery(
						"select model.election.electionId ,model.election.electionDate from ConstituencyElection " +
						" model where model.constituency.constituencyId  = ?");
	...
	public List<Long> getConstituencyElectionsForConstituency(Long constituencyId){
		...
		Query query = getSession().createQuery("select model.constiElecId from ConstituencyElection model" +
				" where model.constituency.constituencyId = ?");
	...
	public List<Object[]> getElectionYearsByConstituencyIds(List<Long> constituencyIds)
	{
		...
		Query query = getSession().createQuery("select distinct model.constituencyElection.election.electionId , model.constituencyElection.election.electionYear from BoothConstituencyElection model " +
				"where model.constituencyElection.constituency.constituencyId in(:constituencyIds) order by model.constituencyElection.election.electionYear desc");
	...
	public List<Object[]> getConstituenciesByElectionId(Long electionId)
	{
		Query query = getSession().createQuery("select distinct model.constituency.constituencyId , model.constituency.name from " +
				" ConstituencyElection model where model.election.electionId = :electionId order by  model.constituency.name asc ");
	...
	public List<Object[]> getConstituenciyListByElectionId(Long electionId)
	{
		Query query = getSession().createQuery("select distinct model.constituency.constituencyId , model.constituency.name from " +
				" ConstituencyElection model where model.election.electionId = :electionId order by  model.constituency.name asc ");
