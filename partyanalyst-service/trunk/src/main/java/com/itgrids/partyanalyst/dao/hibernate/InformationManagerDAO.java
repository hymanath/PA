...
public class InformationManagerDAO extends GenericDaoHibernate<InformationManager, Long> implements IInformationManagerDAO{
	...
	{
		Query query = getSession().createQuery("select model.informationManagerId,model.firstName,model.lastName,model.mobile,model.designation,model.createdTime,model.regionScopes.regionScopesId,model.locationValue from InformationManager model where model.parentUser.user.userId = :userId");
