...
public class StaticDataService implements IStaticDataService {
	...
	private INominationDAO nominationDAO;s
	...
	*//**
	...
	public List<SelectOptionVO> getElectionIdsAndYearsInTehsil(Long electionType,Long tehsilId)
	{
		List<SelectOptionVO> years = new ArrayList<SelectOptionVO>();
		try{
			List<Object[]> elections = villageBoothElectionDAO.findElectionsForElectionTypeInAtehsil(electionType,tehsilId);
	...
	public List<SelectOptionVO> getElectionYearByPartyId(Long stateId,Long partyId,Long electionTypeId)
	{
		try{
			...
			if(list != null && list.size() > 0)
			{
				...
				for(Object[] obj : list)
					electionYears.add(new SelectOptionVO((Long)obj[1],obj[0].toString()));
			}
			return electionYears;			
		}
		catch(Exception e){
...
/*
	...
	*//**
	...
	{
		Long parliamentId =0l;
		List<Long> list = new ArrayList<Long>();
		try{
		list =delimitationConstituencyAssemblyDetailsDAO.findLatestParliamentByAssembly(constituencyId);
			if(list != null && list.size() > 0)
		parliamentId = list.get(0);
		}
